	...
	public override float CalculateDamage(in AttackInformation attackInformation, in AttackCollisionData collisionData, in MissionWeapon weapon, float baseDamage)
	{
		...
		if (!flag6)
		{
			...
			if (attackInformation.DoesAttackerHaveMountAgent && (currentUsageItem2 == null || currentUsageItem2.RelevantSkill != DefaultSkills.Crossbow))
			{
				...
				float value2 = -0.01f * MathF.Max(0f, DefaultSkillEffects.HorseWeaponDamagePenalty.GetPrimaryValue(effectiveSkill));
			...
			if (characterObject2 != null)
			{
				if (currentUsageItem2 != null)
				{
					...
					{
						PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Bow.SkirmishPhaseMaster, characterObject2, isPrimaryBonus: true, ref bonuses);
						PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.Throwing.Skirmisher, characterObject3, ref bonuses);
						if (characterObject2.IsRanged)
						{
							PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.Bow.SkirmishPhaseMaster, characterObject3, ref bonuses);
						}
						if (currentUsageItem != null)
						{
							if (currentUsageItem.WeaponClass == WeaponClass.Crossbow)
							{
								PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Crossbow.CounterFire, characterObject2, isPrimaryBonus: true, ref bonuses);
								PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.Crossbow.CounterFire, characterObject3, ref bonuses);
							}
							else if (currentUsageItem.RelevantSkill == DefaultSkills.Throwing)
							{
								PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Throwing.Skirmisher, characterObject2, isPrimaryBonus: true, ref bonuses);
							}
						}
					...
					else if (currentUsageItem2.IsMeleeWeapon)
					{
						if (characterObject3 != null)
						{
							...
							{
								PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.OneHanded.Basher, characterObject3, ref bonuses);
							}
						}
						if (activeBanner2 != null)
						{
				...
				if (flag5)
				{
					PerkHelper.AddPerkBonusForCharacter(DefaultPerks.OneHanded.SteelCoreShields, characterObject2, isPrimaryBonus: true, ref bonuses);
					if (flag3)
					{
						PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.OneHanded.SteelCoreShields, characterObject3, ref bonuses);
					}
					if (collisionData.AttackBlockedWithShield && !collisionData.CorrectSideShieldBlock)
					{
						PerkHelper.AddPerkBonusForCharacter(DefaultPerks.OneHanded.ShieldWall, characterObject2, isPrimaryBonus: true, ref bonuses);
					}
				}
				if (collisionData.IsHorseCharge)
				{
					PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Polearm.SureFooted, characterObject2, isPrimaryBonus: true, ref bonuses);
					PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Athletics.Braced, characterObject2, isPrimaryBonus: true, ref bonuses);
					if (characterObject3 != null)
					{
						PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.Polearm.SureFooted, characterObject3, ref bonuses);
						PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.Athletics.Braced, characterObject3, ref bonuses);
					}
				}
				if (collisionData.IsFallDamage)
				{
					PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Athletics.StrongLegs, characterObject2, isPrimaryBonus: true, ref bonuses);
				}
				PerkHelper.AddPerkBonusFromCaptain(DefaultPerks.Tactics.EliteReserves, characterObject3, ref bonuses);
			}

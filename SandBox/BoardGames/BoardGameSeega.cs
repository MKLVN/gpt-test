...
public class BoardGameSeega : BoardGameBase
{
	...
	private void RestoreStartingBoard()
	{
		...
		foreach (PawnSeega playerOneUnit in base.PlayerOneUnits)
		{
			...
			else if (!playerOneUnit.Entity.GlobalPosition.NearlyEquals(_startState.PawnInformation[num].Position))
			{
	...
	{
		List<List<Move>> moves = CalculateAllValidMoves((!playerOne) ? BoardGameSide.AI : BoardGameSide.Player);

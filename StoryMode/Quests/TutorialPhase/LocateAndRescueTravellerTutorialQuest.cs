...
public class LocateAndRescueTravellerTutorialQuest : StoryModeQuestBase
{
	...
	private MobileParty CreateRaiderParty()
	{
		...
		MobileParty mobileParty = BanditPartyComponent.CreateBanditParty("locate_and_rescue_traveller_quest_raider_party_" + _raiderPartyCount, settlement.OwnerClan, settlement.Hideout, isBossParty: false);
		...
		troopRoster.AddToCounts(@object, 6);
		CharacterObject object2 = MBObjectManager.Instance.GetObject<CharacterObject>("tutorial_placeholder_volunteer");
		troopRoster2.AddToCounts(object2, (MBRandom.RandomFloat >= 0.5f) ? 1 : 2);
		Settlement object3 = MBObjectManager.Instance.GetObject<Settlement>("village_ES3_2");
		mobileParty.InitializeMobilePartyAroundPosition(troopRoster, troopRoster2, object3.GatePosition, MobileParty.MainParty.SeeingRange * 0.75f);
	...
	{
		AddGameMenu("encounter_raiders_quest", new TextObject("{=mU1bC1mp}You encountered the raider party."), game_menu_encounter_on_init, GameOverlays.MenuOverlayType.Encounter);

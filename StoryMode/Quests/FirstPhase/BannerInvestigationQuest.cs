...
public class BannerInvestigationQuest : StoryModeQuestBase
{
	...
	protected override void SetDialogs()
	{
		...
		Campaign.Current.ConversationManager.AddDialogFlow(DialogFlow.CreateDialogFlow("hero_main_options", 150).BeginPlayerOptions().PlayerSpecialOption(new TextObject("{=qV1e0x8i}What is 'Neretzes' Folly'?"))
			.Condition(() => Hero.OneToOneConversationHero != null && CharacterObject.OneToOneConversationCharacter.Occupation == Occupation.Lord && Hero.OneToOneConversationHero.Clan != Hero.MainHero.Clan && !_battleSummarized)
	...
	{
		if (_battleSummarized && Hero.OneToOneConversationHero != null && CharacterObject.OneToOneConversationCharacter.Occupation == Occupation.Lord && Hero.OneToOneConversationHero.Clan != Hero.MainHero.Clan)
		{
			return !_noblesToTalk.ContainsKey(Hero.OneToOneConversationHero);
		}
		return false;
	}
	...
	private void SetNobleDialogs()
	{
		foreach (KeyValuePair<Hero, bool> item in _noblesToTalk)
		{
			if (!item.Value)
			{
				TextObject answer = TextObject.Empty;

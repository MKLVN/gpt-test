...
public class RichText : IText
{
	...
	public void CalculateTextOutput(bool fixedWidth, bool fixedHeight, float width, float height, SpriteData spriteData, float renderScale)
	{
		...
		for (int i = 0; i < _tokens.Count; i++)
		{
			...
			else if (textToken.Type == TextToken.TokenType.EmptyCharacter)
			{
				...
				if (fixedWidth && flag2 && flag3 && _shouldAddNewLineWhenExceedingContainerWidth)
				{
			...
			else if (textToken.Type == TextToken.TokenType.Character)
			{
				...
				if (fixedWidth && flag4 && _shouldAddNewLineWhenExceedingContainerWidth)
				{
					List<TextToken> list = TextOutput.Tokens.Select((TextTokenOutput t) => t.Token).ToList();
					int indexOfFirstAppropriateCharacterToMoveToNextLineBackwardsFromIndex = TextHelper.GetIndexOfFirstAppropriateCharacterToMoveToNextLineBackwardsFromIndex(list, list.Count - 1, CurrentLanguage);
					int num5 = TextHelper.GetIndexOfFirstAppropriateCharacterToMoveToNextLineForwardsFromIndex(_tokens, i, CurrentLanguage);
					float num6 = 0f;
			...
			else
			{
				...
				if (tag.Name == "img")
				{
					...
					if (fixedWidth && flag6 && flag7 && _shouldAddNewLineWhenExceedingContainerWidth)
					{
						TextOutput.AddNewLine(currentLineEnded: false, num2);
					}
	...
	{
		if (position.X >= 0f && position.Y >= 0f && position.X < (float)Width && position.Y < (float)Height && TextOutput != null)
		{
			foreach (TextTokenOutput token in TextOutput.Tokens)
			{
				if (token.Rectangle.IsPointInside(position))
				{
					return token;
				}
			}

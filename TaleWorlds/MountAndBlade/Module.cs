...
public sealed class Module : DotNetObject, IGameStateManagerOwner
{
	...
	private void FindMissions()
	{
		...
		foreach (Assembly assembly in array)
		{
			...
			{
				object[] customAttributes = item.GetCustomAttributes(typeof(MissionManager), inherit: true);
		...
		foreach (Type item2 in list)
		{
			...
			{
				object[] customAttributes2 = methodInfo.GetCustomAttributes(typeof(MissionMethod), inherit: true);
	...
	{
		TaleWorlds.Library.Debug.FailedAssert("MBThrowException", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Module.cs", "MBThrowException", 1418);
	}
	...
	internal static string CreateProcessedActionSetsXMLForNative()
	{
		...
		for (int i = 0; i < xDocument.Descendants("action_set").Count(); i++)
		{
			for (int j = i + 1; j < xDocument.Descendants("action_set").Count(); j++)
			{
	...
	private void FillMultiplayerGameTypes()
	{
		...
		_multiplayerGameTypes = new List<MultiplayerGameTypeInfo>();
	}

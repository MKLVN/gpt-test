...
public class LineFormation : IFormationArrangement
{
	...
	private static IEnumerable<Vec2i> GetOrderedUnitPositionIndicesAux(int fileIndexBegin, int fileIndexEnd, int rankIndexBegin, int rankIndexEnd)
	{
		...
		int centerFileIndex2;
		if (fileCount % 2 == 1)
		{
			centerFileIndex2 = fileCount / 2;
			for (int rankIndex2 = rankIndexBegin; rankIndex2 <= rankIndexEnd; rankIndex2++)
			{
				yield return new Vec2i(fileIndexBegin + centerFileIndex2, rankIndex2);
				for (int fileIndexOffset2 = 1; fileIndexOffset2 <= centerFileIndex2; fileIndexOffset2++)
				{
					yield return new Vec2i(fileIndexBegin + centerFileIndex2 - fileIndexOffset2, rankIndex2);
					if (centerFileIndex2 + fileIndexOffset2 < fileCount)
					{
						yield return new Vec2i(fileIndexBegin + centerFileIndex2 + fileIndexOffset2, rankIndex2);
					}
				}
		...
		centerFileIndex2 = fileCount / 2 - 1;
		for (int rankIndex2 = rankIndexBegin; rankIndex2 <= rankIndexEnd; rankIndex2++)
		{
			yield return new Vec2i(fileIndexBegin + centerFileIndex2, rankIndex2);
			for (int fileIndexOffset2 = 1; fileIndexOffset2 <= centerFileIndex2 + 1; fileIndexOffset2++)
			{
				yield return new Vec2i(fileIndexBegin + centerFileIndex2 + fileIndexOffset2, rankIndex2);
				if (centerFileIndex2 - fileIndexOffset2 >= 0)
				{
					yield return new Vec2i(fileIndexBegin + centerFileIndex2 - fileIndexOffset2, rankIndex2);
				}
			}
	...
	public List<IFormationUnit> GetUnitsToPop(int count, Vec3 targetPosition)
	{
		...
		if (_unpositionedUnits.Count > 0)
		{
			...
			if (num < _unpositionedUnits.Count)
			{
				...
				{
					if (!(_unpositionedUnits[i] is Agent agent))
					{
						if (flag)
						{
		...
		if (count > 0)
		{
			...
			for (int num6 = FileCount * RankCount - 1; num6 >= 0; num6--)
			{
				...
				if (_units2D[item, item2] != null)
				{
					...
					else if (!(_units2D[item, item2] is Agent agent2))
					{
						if (flag3)
						{
	...
	private static IFormationUnit GetUnitToFillIn(LineFormation formation, int relocationFileIndex, int relocationRankIndex)
	{
		...
		TaleWorlds.Library.Debug.FailedAssert("This line should not be reached.", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\AI\\Formation\\LineFormation.cs", "GetUnitToFillIn", 3155);
		return null;
	}
	...
	{
		if (!arrangementChangeAllowed || !(unit is Agent agent) || unit.FormationRankIndex <= 0 || !agent.HasShieldCached || !(_units2D[unit.FormationFileIndex, unit.FormationRankIndex - 1] is Agent agent2) || agent2.Banner != null)
		{
			return;
		}
		...
		{
			if (unit.FormationFileIndex - i >= 0 && _units2D[unit.FormationFileIndex - i, unit.FormationRankIndex - 1] is Agent agent3 && !agent3.HasShieldCached && agent3.Banner == null)
			{
				SwitchUnitLocations(unit, agent3);
				break;
			}
			if (unit.FormationFileIndex + i < FileCount && _units2D[unit.FormationFileIndex + i, unit.FormationRankIndex - 1] is Agent agent4 && !agent4.HasShieldCached && agent4.Banner == null)
			{

...
public class TacticBreachWalls : TacticComponent
{
	...
	protected internal override void TickOccasionally()
	{
		...
		case TacticState.AssaultUnderRangedCover:
		{
			List<ArcherPosition> archerPositions = DetermineCurrentArcherPositions(currentLanes);
			if (flag || IsTacticReapplyNeeded)
			{
				_cachedUsedArcherPositions = archerPositions;
				WellRoundedAssault(ref currentLanes, ref archerPositions);
				IsTacticReapplyNeeded = false;
			}
			else if (_cachedUsedArcherPositions.Count != archerPositions.Count)
			{
				MergeFormationsIfArcherPositionsBecameUnavailable(ref archerPositions);
			}

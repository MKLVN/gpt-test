...
public class ColumnFormation : IFormationArrangement
{
	...
	{
		if (newPosition.HasValue && RankCount > 0 && (newPosition.Value.AsVec2 - previousPosition).LengthSquared >= RankDepth * RankDepth)
		{
	...
	private void SwitchUnitIfLeftBehind(IFormationUnit unit)
	{
		...
		else
		{
			...
			if (closestUnitTo2 != null && closestUnitTo2 != unit && closestUnitTo2 is Agent agent && agent.IsAIControlled)
			{

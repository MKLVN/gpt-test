...
public sealed class Mission : DotNetObject, IMission
{
	...
	public float GetRemovedAgentRatioForSide(BattleSideEnum side)
	{
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("Cannot get removed agent count for side. Invalid battle side passed!", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "GetRemovedAgentRatioForSide", 634);
		}
	...
	public void AutoDeployTeamUsingTeamAI(Team team, bool enforceNotSplittableByAI = false)
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			formation.SetControlledByAI(isControlledByAI: true, formation.EnforceNotSplittableByAI || enforceNotSplittableByAI);
		}
		...
		{
			Formation formation2 = list[l];
			bool enforceNotSplittableByAI2 = array[l];
			formation2.SetControlledByAI(isControlledByAI: true, enforceNotSplittableByAI2);
		}
	}
	...
	public void AutoDeployTeamUsingDeploymentPlan(Team team)
	{
		...
		for (int j = 0; j < list.Count; j++)
		{
			...
			IFormationDeploymentPlan formationPlan = _deploymentPlan.GetFormationPlan(team.Side, (FormationClass)j, DeploymentPlanType.Initial);
			if (formationPlan.HasDimensions)
			{
				formation2.FormOrder = FormOrder.FormOrderCustom(formationPlan.PlannedWidth);
			}
			GetFormationSpawnFrame(formation2.Team.Side, formation2.FormationIndex, isReinforcement: false, out var spawnPosition, out var spawnDirection);
			formation2.SetMovementOrder(MovementOrder.MovementOrderMove(spawnPosition));
			formation2.FacingOrder = FacingOrder.FacingOrderLookAtDirection(spawnDirection);
			formation2.SetPositioning(spawnPosition, spawnDirection);
		...
		for (int k = 0; k < list.Count; k++)
		{
			list[k].SetControlledByAI(array[k]);
		}
	}
	...
	public MBReadOnlyList<FleePosition> GetFleePositionsForSide(BattleSideEnum side)
	{
		...
		case BattleSideEnum.NumSides:
			TaleWorlds.Library.Debug.FailedAssert("Flee position with invalid battle side field found!", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "GetFleePositionsForSide", 1665);
			return null;
		default:
			num = (int)(side + 1);
			break;
	...
	public void AddFleePosition(FleePosition fleePosition)
	{
		...
		{
		case BattleSideEnum.NumSides:
			TaleWorlds.Library.Debug.FailedAssert("Flee position with invalid battle side field found!", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "AddFleePosition", 1746);
			break;
		case BattleSideEnum.None:
		{
	...
	public void RecalculateBody(ref WeaponData weaponData, ItemComponent itemComponent, WeaponDesign craftedWeaponData, ref WeaponSpawnFlags spawnFlags)
	{
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("Item has no body! Applying a default body, but this should not happen! Check this!", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "RecalculateBody", 2535);
		...
		if (!weaponComponent.Item.ItemFlags.HasAnyFlag(ItemFlags.DoNotScaleBodyAccordingToWeaponLength))
		{
			...
			else
			{
				...
				else
				{
					...
					if (num8 == 0)
					{
						TaleWorlds.Library.Debug.FailedAssert("Item has 0 body parts. Applying a default body, but this should not happen! Check this!", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "RecalculateBody", 2673);
						return;
					}
					...
					case WeaponClass.LargeShield:
						TaleWorlds.Library.Debug.FailedAssert("Shields should not have recalculate body flag.", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "RecalculateBody", 2747);
						break;
					}
				}
			}
	...
	public void GetTroopSpawnFrameWithIndex(AgentBuildData buildData, int troopSpawnIndex, int troopSpawnCount, out Vec3 troopSpawnPosition, out Vec2 troopSpawnDirection)
	{
		...
		else
		{
			...
			if (buildData.AgentSpawnsUsingOwnTroopClass)
			{
				...
				GetFormationSpawnFrame(side, agentTroopClass, agentIsReinforcement, out formationPosition, out formationDirection);
			}
			...
			{
				GetFormationSpawnFrame(side, FormationClass.NumberOfRegularFormations, agentIsReinforcement, out formationPosition, out formationDirection);
			}
			else
			{
				GetFormationSpawnFrame(side, agentFormation.FormationIndex, agentIsReinforcement, out formationPosition, out formationDirection);
			}
		}
		bool isMountedFormation = !buildData.AgentNoHorses && agentFormation.HasAnyMountedUnit;
		agentFormation.GetUnitSpawnFrameWithIndex(troopSpawnIndex, in formationPosition, in formationDirection, agentFormation.Width, troopSpawnCount, agentFormation.UnitSpacing, isMountedFormation, out var unitSpawnPosition, out var unitSpawnDirection);
		...
		if (unitSpawnPosition.HasValue)
		{
			...
			{
				troopSpawnPosition = Scene.GetLastPointOnNavigationMeshFromWorldPositionToDestination(ref formationPosition, unitSpawnPosition.Value.AsVec2);
			}
			else
			{
	...
	public Agent SpawnAgent(AgentBuildData agentBuildData, bool spawnFromAgentVisuals = false)
	{
		...
		if (!agentBuildData.AgentInitialPosition.HasValue)
		{
			...
			Vec2 troopSpawnDirection = Vec2.Invalid;
			...
			else if (agentFormation != null)
			{
				...
				GetTroopSpawnFrameWithIndex(agentBuildData, num2, num3, out troopSpawnPosition, out troopSpawnDirection);
			}
			else
			{
				GetFormationSpawnFrame(side, FormationClass.NumberOfAllFormations, agentBuildData.AgentIsReinforcement, out var spawnPosition, out troopSpawnDirection);
				troopSpawnPosition = spawnPosition.GetGroundVec3();
			}
			agentBuildData.InitialPosition(in troopSpawnPosition).InitialDirection(in troopSpawnDirection);
		}
	...
	public void SetInitialAgentCountForSide(BattleSideEnum side, int agentCount)
	{
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("Cannot set initial agent count.", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "SetInitialAgentCountForSide", 4062);
		}
	}
	...
	public Agent SpawnTroop(IAgentOriginBase troopOrigin, bool isPlayerSide, bool hasFormation, bool spawnWithHorse, bool isReinforcement, int formationTroopCount, int formationTroopIndex, bool isAlarmed, bool wieldInitialWeapons, bool forceDismounted, Vec3? initialPosition, Vec2? initialDirection, string specialActionSetSuffix = null, ItemObject bannerItem = null, FormationClass formationIndex = FormationClass.NumberOfAllFormations, bool useTroopClassForSpawn = false)
	{
		...
		if (bannerItem != null)
		{
			...
			{
				TaleWorlds.Library.Debug.FailedAssert(string.Concat("Passed banner item with name: ", bannerItem.Name, " is not a proper banner item"), "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "SpawnTroop", 4169);
	...
	public void RemoveMissionBehavior(MissionBehavior missionBehavior)
	{
		...
		case MissionBehaviorType.Other:
			...
			break;
		default:
			TaleWorlds.Library.Debug.FailedAssert("Invalid behavior type", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "RemoveMissionBehavior", 4463);
			break;
		}
	...
	public void JoinEnemyTeam()
	{
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("Player is neither attacker nor defender.", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "JoinEnemyTeam", 4507);
		}
	}
	...
	public static Team GetAgentTeam(IAgentOriginBase troopOrigin, bool isPlayerSide)
	{
		if (Current == null)
		{
			TaleWorlds.Library.Debug.FailedAssert("Mission current is null", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\Mission.cs", "GetAgentTeam", 5198);
			return null;
		}
		Team team = null;
	...
	private MatrixFrame CalculateAttachedLocalFrame(in MatrixFrame attachedGlobalFrame, AttackCollisionData collisionData, WeaponComponentData missileWeapon, Agent affectedAgent, GameEntity hitEntity, Vec3 missileMovementVelocity, Vec3 missileRotationSpeed, MatrixFrame shieldGlobalFrame, bool shouldMissilePenetrate)
	{
		...
		{
			matrixFrame = ((collisionData.CollisionBoneIndex < 0) ? hitEntity.GetGlobalFrame().TransformToLocalNonOrthogonal(ref matrixFrame) : hitEntity.Skeleton.GetBoneEntitialFrameWithIndex(collisionData.CollisionBoneIndex).TransformToLocalNonOrthogonal(ref matrixFrame));
		}
		else
		{
	...
	private CombatLogData GetAttackCollisionResults(Agent attackerAgent, Agent victimAgent, GameEntity hitObject, float momentumRemaining, in MissionWeapon attackerWeapon, bool crushedThrough, bool cancelDamage, bool crushedThroughWithoutAgentCollision, ref AttackCollisionData attackCollisionData, out WeaponComponentData shieldOnBack, out CombatLogData combatLog)
	{
		...
		if (num > 0f)
		{
			if (attackCollisionData.AttackBlockedWithShield && combatPerkHandler != null)
			{
				...
				num2 = TaleWorlds.Library.MathF.Max(0f, num2 * num5);
			}
			...
			{
				float num6 = TaleWorlds.Library.MathF.Max(0f, 1f + combatPerkHandler.GetDamage(attackerWeapon.CurrentUsageItem, combatLog.DamageType, attackCollisionData.IsAlternativeAttack) + combatPerkHandler.GetDamageTaken(attackerWeapon.CurrentUsageItem, combatLog.DamageType));
		...
		if (!attackCollisionData.IsFallDamage && attackInformation.IsFriendlyFire)
		{
			if (!attackInformation.IsAttackerAIControlled && GameNetwork.IsSessionActive)
			{
				...
				attackCollisionData.SelfInflictedDamage = TaleWorlds.Library.MathF.Round((float)attackCollisionData.InflictedDamage * ((float)num7 * 0.01f));
				...
				attackCollisionData.InflictedDamage = TaleWorlds.Library.MathF.Round((float)attackCollisionData.InflictedDamage * ((float)num8 * 0.01f));

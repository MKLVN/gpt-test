...
public class MultiplayerGameNotificationsComponent : MissionNetwork
{
	...
	private void ShowNotification(MultiplayerNotificationEnum notification, params int[] parameters)
	{
		...
		NotificationProperty notificationProperty = (NotificationProperty)notification.GetType().GetField(notification.ToString()).GetCustomAttributes(typeof(NotificationProperty), inherit: false)
			.Single();

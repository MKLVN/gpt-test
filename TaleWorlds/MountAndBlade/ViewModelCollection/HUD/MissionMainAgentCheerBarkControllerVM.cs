...
public class MissionMainAgentCheerBarkControllerVM : ViewModel
{
	...
	private List<(string, int)> _playerTauntsWithIndices;
	...
	private void PopulateList()
	{
		...
		if (flag)
		{
			...
			CheerBarkNodeItemVM cheerBarkNodeItemVM = new CheerBarkNodeItemVM(new TextObject("{=*}Taunt"), "cheer", hotKey2);
			Nodes.Add(cheerBarkNodeItemVM);
			TauntCosmeticElement[] array = new TauntCosmeticElement[TauntCosmeticElement.MaxNumberOfTaunts];
			foreach (var playerTauntsWithIndex in _playerTauntsWithIndices)
			{
				string item = playerTauntsWithIndex.Item1;
				int item2 = playerTauntsWithIndex.Item2;
				TauntCosmeticElement tauntCosmeticElement = CosmeticsManager.GetCosmeticElement(item) as TauntCosmeticElement;
				if ((tauntCosmeticElement.IsFree || _ownedTauntCosmetics.Contains(item)) && item2 >= 0 && item2 < TauntCosmeticElement.MaxNumberOfTaunts)
				{
	...
	private async void UpdatePlayerTauntIndices()
	{
		...
		{
			_playerTauntsWithIndices = await TauntCosmeticElement.GetTauntIndicesForPlayerAsync(NetworkMain.GameClient.PlayerData.UserId.ToString());
		}
		if (_playerTauntsWithIndices == null)
		{
			_playerTauntsWithIndices = new List<(string, int)>();
		}
	}

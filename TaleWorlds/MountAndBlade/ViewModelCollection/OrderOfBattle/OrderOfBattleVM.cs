...
public class OrderOfBattleVM : ViewModel
{
	...
	private List<(OrderOfBattleHeroItemVM Hero, bool WasCommander)> ClearAllHeroAssignments()
	{
		...
		for (int i = 0; i < _allFormations.Count; i++)
		{
			...
			for (int j = 0; j < _allFormations[i].HeroTroops.Count; j++)
			{
				OrderOfBattleHeroItemVM orderOfBattleHeroItemVM = _allFormations[i].HeroTroops[j];
	...
	private void SetInitialHeroFormations()
	{
		for (int i = 0; i < _allHeroes.Count; i++)
		{
			...
			{
				TaleWorlds.Library.Debug.FailedAssert("Failed to find an initial formation for hero: " + _allHeroes[i].Agent.Name, "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade.ViewModelCollection\\OrderOfBattle\\OrderOfBattleVM.cs", "SetInitialHeroFormations", 623);
			}
		}
	}
	...
	private void RefreshWeights()
	{
		...
		{
			fc.SetWeightAdjustmentLock(isLocked: true);
			float num2 = OrderOfBattleUIHelper.GetCountOfRealUnitsInClass(fc);
			float num3 = allClasses.Where((OrderOfBattleFormationClassVM c) => c.Class == fc.Class).Sum((OrderOfBattleFormationClassVM c) => OrderOfBattleUIHelper.GetCountOfRealUnitsInClass(c));
			fc.Weight = TaleWorlds.Library.MathF.Round(num2 / num3 * 100f);
			fc.IsLocked = !IsPlayerGeneral;
			fc.SetWeightAdjustmentLock(isLocked: false);
		});
		FormationClass fc2;
		for (fc2 = FormationClass.Infantry; fc2 < FormationClass.NumberOfDefaultFormations; fc2++)
		{
			IEnumerable<OrderOfBattleFormationClassVM> source = allClasses.Where((OrderOfBattleFormationClassVM c) => c.Class == fc2);
			OrderOfBattleFormationClassVM orderOfBattleFormationClassVM = source.FirstOrDefault();
			if (source.Count() <= 1)
			{
				continue;
			}
			for (int num = source.Sum((OrderOfBattleFormationClassVM item) => item.Weight); num != 100; num = source.Sum((OrderOfBattleFormationClassVM item) => item.Weight))
			{
				orderOfBattleFormationClassVM.SetWeightAdjustmentLock(isLocked: true);
				orderOfBattleFormationClassVM.Weight += ((num <= 100) ? 1 : (-1));
				orderOfBattleFormationClassVM.SetWeightAdjustmentLock(isLocked: false);
			}
		}
		allClasses.ForEach(delegate(OrderOfBattleFormationClassVM fc)
		{
	...
	private void DistributeTroops(OrderOfBattleFormationClassVM formationClass)
	{
		...
		{
			_orderController.RearrangeFormationsAccordingToFilters(Agent.Main.Team, massTransferDataForFormation);
	...
	{
		List<OrderOfBattleFormationClassVM> matchingClasses = OrderOfBattleUIHelper.GetMatchingClasses(_allFormations, formationClass, (OrderOfBattleFormationClassVM fc) => !fc.IsLocked);
		...
		matchingClasses.Remove(formationClass);
		int changePerClass = TaleWorlds.Library.MathF.Round((float)(formationClass.PreviousWeight - formationClass.Weight) / (float)matchingClasses.Count);
		...
		while (matchingClasses.Count > 0 && num2 != 100)
		{
			...
			if (num2 > 100)
			{
				OrderOfBattleFormationClassVM formationClassWithExtremumWeight = OrderOfBattleUIHelper.GetFormationClassWithExtremumWeight(matchingClasses, isMinimum: false);
			...
			{
				OrderOfBattleFormationClassVM formationClassWithExtremumWeight2 = OrderOfBattleUIHelper.GetFormationClassWithExtremumWeight(matchingClasses, isMinimum: true);
			...
			if (num3 == num2)
			{
				TaleWorlds.Library.Debug.FailedAssert("Failed to sum up all weights to 100", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade.ViewModelCollection\\OrderOfBattle\\OrderOfBattleVM.cs", "DistributeWeights", 1162);
				break;
			}
		}
		matchingClasses.ForEach(delegate(OrderOfBattleFormationClassVM formation)
	...
	private void DistributeAllTroops()
	{
		...
		for (int i = 0; i < _allFormations.Count; i++)
		{
			...
			if (list.Count > 0)
			{
				_orderController.RearrangeFormationsAccordingToFilters(Agent.Main.Team, list);
			}
			list.Clear();
			if (list2.Count == 4)
			{
	...
	private void TransferAllAvailableTroopsToFormation(OrderOfBattleFormationItemVM formation, FormationClass formationClass)
	{
		...
		{
			_orderController.RearrangeFormationsAccordingToFilters(Agent.Main.Team, massTransferDataForFormationClass);

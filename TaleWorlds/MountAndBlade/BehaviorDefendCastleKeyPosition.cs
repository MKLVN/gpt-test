...
public class BehaviorDefendCastleKeyPosition : BehaviorComponent
{
	...
	private void ResetOrderPositions()
	{
		...
		else
		{
			...
			{
				IEnumerable<IPrimarySiegeWeapon> source = _teamAISiegeDefender.PrimarySiegeWeapons.Where((IPrimarySiegeWeapon sw) => sw.WeaponSide == _behaviorSide && ((sw is SiegeWeapon siegeWeapon && !siegeWeapon.IsDestroyed && !siegeWeapon.IsDeactivated) || sw.HasCompletedAction()));
				if (!source.Any())
				{

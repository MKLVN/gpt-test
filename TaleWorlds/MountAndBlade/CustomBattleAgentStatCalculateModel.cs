...
public class CustomBattleAgentStatCalculateModel : AgentStatCalculateModel
{
	...
	private void UpdateHumanStats(Agent agent, AgentDrivenProperties agentDrivenProperties)
	{
		...
		if (weaponComponentData != null)
		{
			...
			if (agent.HasMount)
			{
				float num5 = 1f - TaleWorlds.Library.MathF.Max(0f, 0.2f - (float)effectiveSkill2 * 0.002f);
				agentDrivenProperties.SwingSpeedMultiplier *= num5;
				agentDrivenProperties.ThrustOrRangedReadySpeedMultiplier *= num5;
				agentDrivenProperties.ReloadSpeed *= num5;
			}
		}
		...
		float num6 = 0.7f + 0.00070000015f * (float)effectiveSkill;
		float num7 = TaleWorlds.Library.MathF.Max(0.2f * (1f - (float)effectiveSkill * 0.001f), 0f) * num2 / (float)weight;
		float num8 = MBMath.ClampFloat(num6 - num7, 0f, 0.91f);
		agentDrivenProperties.MaxSpeedMultiplier = GetEnvironmentSpeedFactor(agent) * num8;
		...
		float num9 = agent.MountAgent?.GetAgentDrivenPropertyValue(DrivenProperty.AttributeRiding) ?? 1f;
		agentDrivenProperties.AttributeRiding = (float)effectiveSkill2 * num9;
		...
		if (!agent.Mission.Scene.IsAtmosphereIndoor)
		{
			...
			{
				num10 += TaleWorlds.Library.MathF.Min(0.3f, rainDensity + fog);
			}
		...
		agentDrivenProperties.AiShooterError *= num10;
	}
	...
	private void GetBannerEffectsOnAgent(Agent agent, AgentDrivenProperties agentDrivenProperties, WeaponComponentData rightHandEquippedItem)
	{
		...
		{
			bool num = rightHandEquippedItem?.IsRangedWeapon ?? false;
			...
			if (num && rightHandEquippedItem != null)
			{

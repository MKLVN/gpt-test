...
public class SiegeLane
{
	...
	public bool CalculateIsLaneUnusable()
	{
		...
		if (HasGate)
		{
			...
			{
				if (DefensePoints[i] is CastleGate castleGate && castleGate.IsGateOpen && castleGate.GameEntity.HasTag("outer_gate"))
				{
					return false;
				}
			}
		...
		for (int j = 0; j < PrimarySiegeWeapons.Count; j++)
		{
			...
			if ((!(primarySiegeWeapon is UsableMachine usableMachine) || usableMachine.GameEntity != null) && (!(primarySiegeWeapon is SiegeTower siegeTower) || !siegeTower.IsDestroyed) && (primarySiegeWeapon.HasCompletedAction() || !(primarySiegeWeapon is BatteringRam batteringRam) || !batteringRam.IsDestroyed))
			{
				return false;
			}
		}
	...
	public float CalculateLaneCapacity()
	{
		...
		for (int i = 0; i < DefensePoints.Count; i++)
		{
			if (DefensePoints[i] is WallSegment wallSegment && wallSegment.IsBreachedWall)
			{
				flag = true;
				break;
			}
		...
		if (HasGate)
		{
			...
			{
				if (DefensePoints[j] is CastleGate castleGate && !castleGate.IsGateOpen)
				{
					flag2 = false;
					break;
				}
	...
	public void RefreshLane()
	{
		...
		{
			if (PrimarySiegeWeapons[num] is SiegeWeapon siegeWeapon && siegeWeapon.IsDisabled)
			{

...
public static class MissionCombatMechanicsHelper
{
	...
	public static bool DecideAgentShrugOffBlow(Agent victimAgent, AttackCollisionData collisionData, in Blow blow)
	{
		...
		if (victimAgent.Health - (float)collisionData.InflictedDamage >= 1f)
		{
			...
			float num2 = ManagedParameters.Instance.GetManagedParameter(managedParameterEnum) * num;
			float? num3 = MPPerkObject.GetPerkHandler(victimAgent)?.GetDamageInterruptionThreshold();
			if (num3.HasValue && num3.Value > 0f)
			{
				num2 = num3.Value;
			}
			result = (float)collisionData.InflictedDamage <= num2;
		}
		return result;
	}
	...
	internal static void GetDefendCollisionResults(Agent attackerAgent, Agent defenderAgent, CombatCollisionResult collisionResult, int attackerWeaponSlotIndex, bool isAlternativeAttack, StrikeType strikeType, Agent.UsageDirection attackDirection, float collisionDistanceOnWeapon, float attackProgress, bool attackIsParried, bool isPassiveUsageHit, bool isHeavyAttack, ref float defenderStunPeriod, ref float attackerStunPeriod, ref bool crushedThrough, ref bool chamber)
	{
		...
		MissionGameModels.Current.AgentApplyDamageModel.CalculateCollisionStunMultipliers(attackerAgent, defenderAgent, isAlternativeAttack, collisionResult, weaponComponentData, weaponComponentData2, out var attackerStunMultiplier, out var defenderStunMultiplier);
	...
	private static void ComputeBlowDamage(in AttackInformation attackInformation, in AttackCollisionData attackCollisionData, WeaponComponentData attackerWeapon, DamageTypes damageType, float magnitude, int speedBonus, bool cancelDamage, out int inflictedDamage, out int absorbedByArmor)
	{
		...
		int num4 = MBMath.ClampInt(TaleWorlds.Library.MathF.Ceiling(Game.Current.BasicModels.StrikeMagnitudeModel.ComputeRawDamage(damageType, magnitude, 0f, absorbedDamageRatio) * num3), 0, 2000);
	...
	private static void ComputeBlowDamageOnShield(in AttackInformation attackInformation, in AttackCollisionData attackCollisionData, WeaponComponentData attackerWeapon, float blowMagnitude, out int inflictedDamage)
	{
		...
		float num = Game.Current.BasicModels.StrikeMagnitudeModel.ComputeRawDamage(damageType, blowMagnitude, getModifiedArmorForCurrentUsage, absorbedDamageRatio);
		if (attackCollisionData.IsMissile)
		{
			num = ((attackerWeapon.WeaponClass == WeaponClass.Javelin) ? (num * 0.25f) : ((attackerWeapon.WeaponClass == WeaponClass.ThrowingAxe) ? (num * 0.5f) : ((!attackerWeapon.WeaponFlags.HasAnyFlag(WeaponFlags.CanPenetrateShield)) ? (num * 0.1f) : (num * 0.5f))));
		}
	...
	public static float CalculateBaseMeleeBlowMagnitude(in AttackInformation attackInformation, in MissionWeapon weapon, StrikeType strikeType, float progressEffect, float impactPointAsPercent, float exraLinearSpeed, bool doesAttackerHaveMount)
	{
		...
		if (strikeType == StrikeType.Thrust)
		{
			...
			float thrustWeaponSpeed = (float)weapon.GetModifiedThrustSpeedForCurrentUsage() / 11.764706f * num;
			return Game.Current.BasicModels.StrikeMagnitudeModel.CalculateStrikeMagnitudeForThrust(attackerAgentCharacter, attackerCaptainCharacter, thrustWeaponSpeed, weapon.Item.Weight, currentUsageItem, exraLinearSpeed, doesAttackerHaveMount);
		}
	...
	private static void ComputeBlowMagnitudeMelee(in AttackInformation attackInformation, in AttackCollisionData acd, float momentumRemaining, bool cancelDamage, bool hitWithAnotherBone, StrikeType strikeType, Agent.UsageDirection attackDirection, in MissionWeapon weapon, bool attackerIsDoingPassiveAttack, Vec2 attackerVelocity, Vec2 victimVelocity, out float baseMagnitude, out float specialMagnitude, out float movementSpeedDamageModifier, out int speedBonusInt)
	{
		...
		if (weapon.IsEmpty)
		{
			baseMagnitude = SpeedGraphFunction(acd.AttackProgress, strikeType, attackDirection) * momentumRemaining * ManagedParameters.Instance.GetManagedParameter(ManagedParametersEnum.FistFightDamageMultiplier);
			specialMagnitude = baseMagnitude;
			return;
		}
		...
		float impactPointAsPercent = MBMath.ClampFloat(acd.CollisionDistanceOnWeapon, -0.2f, num4) / num4;
		BasicCharacterObject attackerAgentCharacter = attackInformation.AttackerAgentCharacter;
		...
		if (attackerIsDoingPassiveAttack)
		{
			...
			baseMagnitude = MissionGameModels.Current.AgentApplyDamageModel.CalculatePassiveAttackDamage(attackerAgentCharacter, in acd, baseMagnitude);
		}
		else
		{
			float num5 = SpeedGraphFunction(acd.AttackProgress, strikeType, attackDirection);
			baseMagnitude = CalculateBaseMeleeBlowMagnitude(in attackInformation, in weapon, strikeType, num5, impactPointAsPercent, num3, doesAttackerHaveMountAgent);
			if (baseMagnitude >= 0f && num5 > 0.7f)
			{
				float baseMagnitudeWithoutSpeedBonus = CalculateBaseMeleeBlowMagnitude(in attackInformation, in weapon, strikeType, num5, impactPointAsPercent, 0f, doesAttackerHaveMountAgent);
		...
		specialMagnitude = ConvertBaseAttackMagnitude(currentUsageItem, strikeType, baseMagnitude);
	}
	...
	private static void ComputeBlowMagnitudeMissile(in AttackInformation attackInformation, in AttackCollisionData acd, in MissionWeapon weapon, float momentumRemaining, in Vec2 victimVelocity, out float baseMagnitude, out float specialMagnitude)
	{
		BasicCharacterObject attackerAgentCharacter = attackInformation.AttackerAgentCharacter;
		BasicCharacterObject attackerCaptainCharacter = attackInformation.AttackerCaptainCharacter;
		float missileSpeed = (attackInformation.IsVictimAgentNull ? acd.MissileVelocity.Length : (victimVelocity.ToVec3() - acd.MissileVelocity).Length);
		baseMagnitude = Game.Current.BasicModels.StrikeMagnitudeModel.CalculateStrikeMagnitudeForMissile(attackerAgentCharacter, attackerCaptainCharacter, acd.MissileTotalDamage, missileSpeed, acd.MissileStartingBaseSpeed, weapon.CurrentUsageItem);
		baseMagnitude *= momentumRemaining;
		if (attackInformation.AttackerAgent != null)
		{

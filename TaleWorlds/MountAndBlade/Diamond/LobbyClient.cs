...
public class LobbyClient : Client<LobbyClient>, IChatClientHandler
{
	...
	public async Task<bool> RequestJoinCustomGame(CustomBattleId serverId, string password)
	{
		...
		SendMessage(new RequestJoinCustomGameMessage(serverId, password2));
	...
	private void OnBattleOverMessage(BattleOverMessage message)
	{
		...
		{
			CurrentState = State.AtLobby;
			_handler.OnMatchmakerGameOver(message.OldExperience, message.NewExperience, message.EarnedBadges, message.GoldGained, message.OldInfo, message.NewInfo);
		}
	}
	...
	private void OnJoinCustomGameResultMessage(JoinCustomGameResultMessage message)
	{
		...
		if (_handler != null)
		{
			_handler.OnJoinCustomGameResponse(message.Success, message.JoinGameData, message.Response);
		}
	}
	...
	private void OnClientWantsToConnectCustomGameMessage(ClientWantsToConnectCustomGameMessage message)
	{
		...
		for (int i = 0; i < playerJoinGameData.Length; i++)
		{
			...
			if (_handler != null)
			{
				PlayerJoinGameResponseDataFromHost[] collection = _handler.OnClientWantsToConnectCustomGame(list2.ToArray(), message.Password);
	...
	{
		CurrentState = State.AtLobby;
		_handler.OnMatchmakerGameOver(message.OldExperience, message.NewExperience, new List<string>(), message.GoldGain, null, null);
	}
	...
	{
		return (await CallFunction<GetRankedLeaderboardMessageResult>(new GetRankedLeaderboardMessage(gameType)))?.LeaderboardPlayers;
	}
	...
	public async Task<(bool isSuccessful, int finalGold)> BuyCosmetic(string cosmeticId)
	{
		...
		return (buyCosmeticMessageResult?.Successful ?? false, buyCosmeticMessageResult?.Gold ?? 0);
	}
	...
	public async Task<(bool isSuccessful, List<string> ownedCosmetics, Dictionary<string, List<string>> usedCosmetics)> GetCosmeticsInfo()
	{
		...
		return (getUserCosmeticsInfoMessageResult?.Successful ?? false, getUserCosmeticsInfoMessageResult?.OwnedCosmetics, getUserCosmeticsInfoMessageResult?.UsedCosmetics);
	}
	...
	{
		(PendingRequest requestType, PlayerId playerId) key = (requestType, playerId);
		try
		{

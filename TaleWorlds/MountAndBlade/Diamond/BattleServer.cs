...
public class BattleServer : Client<BattleServer>
{
	...
	{
		SendMessage(new BattleCancelledDueToPlayerQuitMessage(leaverID));
	}
	...
	public void BattleStarted(BattleResult battleResult)
	{
		...
		{
			dictionary.Add(PlayerId.FromString(playerEntry.Key), playerEntry.Value.TeamNo);
		}
		SendMessage(new BattleStartedMessage(dictionary));
	}

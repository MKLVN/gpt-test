...
public class GauntletOptionsScreen : ScreenBase
{
	...
	private void SetHotKey(Key key)
	{
		...
		{
			GameKeyGroupVM gameKeyGroupVM = _dataSource.GameKeyOptionGroups.GameKeyGroups.First((GameKeyGroupVM g) => g.GameKeys.Contains(gameKey));
			if (_gauntletLayer.Input.IsHotKeyReleased("Exit"))
			{
		...
		{
			AuxiliaryKeyGroupVM auxiliaryKeyGroupVM = _dataSource.GameKeyOptionGroups.AuxiliaryKeyGroups.First((AuxiliaryKeyGroupVM g) => g.HotKeys.Contains(auxiliaryKey));
			if (_gauntletLayer.Input.IsHotKeyReleased("Exit"))
			{
				_keybindingPopup.OnToggle(isActive: false);
				return;

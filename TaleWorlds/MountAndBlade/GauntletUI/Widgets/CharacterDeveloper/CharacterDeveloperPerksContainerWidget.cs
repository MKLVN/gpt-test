...
public class CharacterDeveloperPerksContainerWidget : Widget
{
	...
	private void RefreshScopes()
	{
		...
		{
			base.EventManager.RemoveNavigationScope(navigationScope);
		}
		...
		base.EventManager.AddNavigationScope(gamepadNavigationScope, initialize: true);
		int num = -1;
		...
		for (int i = 0; i < _perkWidgets.Count; i++)
		{
			if (_perkWidgets[i].AlternativeType == 0 || num == 0)
			{
				...
				base.EventManager.AddNavigationScope(gamepadNavigationScope2, initialize: true);
			}

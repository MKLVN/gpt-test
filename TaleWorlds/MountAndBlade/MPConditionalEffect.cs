...
public class MPConditionalEffect
{
	...
	public void OnEvent(bool isWarmup, MissionPeer peer, ConditionalEffectContainer container)
	{
		...
		if (!flag)
		{
			...
			{
				MBList<IFormationUnit> mBList = peer?.ControlledFormation?.Arrangement.GetAllUnits();
				if (mBList == null)
				{
					return;
				}
				{
					foreach (IFormationUnit item in mBList)
					{
		...
		if (MultiplayerOptions.OptionType.NumberOfBotsPerFormation.GetIntValue() > 0)
		{
			MBList<IFormationUnit> mBList2 = peer?.ControlledFormation?.Arrangement.GetAllUnits();
			if (mBList2 == null)
			{
				return;
			}
			{
				foreach (IFormationUnit item2 in mBList2)
				{

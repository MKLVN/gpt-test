			...
			if (!MBEditor.IsEditModeOn)
			{
				...
				if (!GameNetwork.IsMultiplayer && Mission.Mode == MissionMode.Deployment)
				{
					...
					if (deploymentPlan.HasDeploymentBoundaries(side, DeploymentPlanType.Initial))
					{
						...
						if (!deploymentPlan.IsPositionInsideDeploymentBoundaries(side, in position, DeploymentPlanType.Initial))
						{
							ref Vec3 origin = ref cameraFrame.origin;
							position = cameraFrame.origin.AsVec2;
							origin.AsVec2 = deploymentPlan.GetClosestDeploymentBoundaryPosition(side, in position, DeploymentPlanType.Initial);
						}
					}
				}
	...
	void IMissionListener.OnInitialDeploymentPlanMade(BattleSideEnum battleSide, bool isFirstPlan)
	{
		if (!GameNetwork.IsMultiplayer && Mission.Mode == MissionMode.Deployment && isFirstPlan)
		{
			...
			if (side == battleSide)
			{
				...
				{
					MatrixFrame battleSideDeploymentFrame = Mission.DeploymentPlan.GetBattleSideDeploymentFrame(side, DeploymentPlanType.Initial);

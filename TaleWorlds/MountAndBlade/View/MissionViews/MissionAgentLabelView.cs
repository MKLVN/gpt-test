...
public class MissionAgentLabelView : MissionView
{
	...
	{
		if (affectedAgent.IsHuman && _agentMeshes.ContainsKey(affectedAgent))
		{
			if (affectedAgent.AgentVisuals != null)
			{
				affectedAgent.AgentVisuals.ReplaceMeshWithMesh(_agentMeshes[affectedAgent], null, BodyMeshTypes.Label);
			}
			_agentMeshes.Remove(affectedAgent);
		}
	}

...
public static class ViewCreatorManager
{
	...
	private static void CheckOverridenViews(Assembly assembly)
	{
		foreach (Type item in assembly.GetTypesSafe())
		{
			...
			if (customAttributes != null && customAttributes.Length == 1 && customAttributes[0] is OverrideView overrideView)
			{
	...
	private static void CollectDefaults(Assembly assembly)
	{
		foreach (Type item in assembly.GetTypesSafe())
		{
			if (typeof(MissionBehavior).IsAssignableFrom(item) && item.GetCustomAttributes(typeof(DefaultView), inherit: false).Length == 1)
			{

...
public class ItemTableau
{
	...
	private void RefreshItemTableau()
	{
		...
		if (_itemTableauEntity == null)
		{
			MatrixFrame frame = _itemRosterElement.GetItemFrameForItemTooltip();
			frame.origin.z += 2.5f;
			...
			if (!string.IsNullOrEmpty(_bannerCode))
			{
				...
				color2 = BannerManager.ColorPalette[banner.BannerDataList[1].ColorId].Color;
			}
			if (itemMeshForInventory != null)
			{
				if (itemType == ItemObject.ItemTypeEnum.HandArmor)
				{
					...
					_itemTableauEntity.Skeleton.TickAnimationsAndForceUpdate(0.01f, frame, tickAnimsForChildren: true);
				}
				...
				{
					_itemTableauEntity = _tableauScene.AddItemEntity(ref frame, itemMeshForInventory);

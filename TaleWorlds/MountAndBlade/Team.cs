...
public class Team : IMissionTeam
{
	...
	{
		this.OnOrderIssued?.Invoke(orderType, appliedFormations, delegateParams);
	}
	...
	public void RearrangeFormationsAccordingToFilters(List<Tuple<Formation, int, TroopFilter, List<Agent>>> MassTransferData)
	{
		...
		list3 = MassTransferData.SelectMany((Tuple<Formation, int, TroopFilter, List<Agent>> mtd) => mtd.Item1.LooseDetachedUnits.Concat(mtd.Item1.Arrangement.GetAllUnits()).Except(mtd.Item4)).ToList();

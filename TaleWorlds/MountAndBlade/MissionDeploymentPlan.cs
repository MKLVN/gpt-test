...
public class MissionDeploymentPlan : IMissionDeploymentPlan
{
	...
	public bool IsPositionInsideDeploymentBoundaries(BattleSideEnum battleSide, in Vec2 position, DeploymentPlanType planType)
	{
		...
		{
			return battleSideDeploymentPlan.IsPositionInsideInitialDeploymentBoundaries(in position);
		}
	...
	public Vec2 GetClosestDeploymentBoundaryPosition(BattleSideEnum battleSide, in Vec2 position, DeploymentPlanType planType)
	{
		...
		if (battleSideDeploymentPlan.HasDeploymentBoundaries(planType))
		{
			return battleSideDeploymentPlan.GetClosestInitialDeploymentBoundaryPosition(in position);
		}
	...
	{
		return _battleSideDeploymentPlans[(int)side].HasDeploymentBoundaries(planType);
	}
	...
	public MatrixFrame GetBattleSideDeploymentFrame(BattleSideEnum side, DeploymentPlanType planType)
	{
		...
		{
			return _battleSideDeploymentPlans[(int)side].GetDeploymentFrame(planType);
		}
	...
	{
		if (HasDeploymentBoundaries(side, planType))
		{
			return _battleSideDeploymentPlans[(int)side].GetDeploymentBoundaries(planType);
		}
	...
	{
		return IsPositionInsideDeploymentBoundaries(battleSide, in position, planType);
	}
	...
	{
		return GetClosestDeploymentBoundaryPosition(battleSide, in position, planType);
	}
}

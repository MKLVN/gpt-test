		...
		foreach (Assembly assembly in assemblies)
		{
			try
			{
				...
				for (int j = 0; j < types.Length; j++)
				{
					...
					{
						object[] customAttributes = methodInfo.GetCustomAttributes(typeof(CrashInformationProvider), inherit: false);
						if (customAttributes != null && customAttributes.Length != 0 && customAttributes[0] is CrashInformationProvider && methodInfo.Invoke(null, new object[0]) is CrashInformation item)
						{
							list.Add(item);
						}
					}

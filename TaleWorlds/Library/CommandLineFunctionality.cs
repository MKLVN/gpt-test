	...
	public static List<string> CollectCommandLineFunctions()
	{
		...
		foreach (Assembly assembly in assemblies)
		{
			...
			foreach (Type item in assembly.GetTypesSafe())
			{
				...
				{
					object[] customAttributes = methodInfo.GetCustomAttributes(typeof(CommandLineArgumentFunction), inherit: false);
					if (customAttributes != null && customAttributes.Length != 0 && customAttributes[0] is CommandLineArgumentFunction commandLineArgumentFunction && !(methodInfo.ReturnType != typeof(string)))
					{

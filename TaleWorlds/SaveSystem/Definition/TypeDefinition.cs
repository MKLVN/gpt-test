...
public class TypeDefinition : TypeDefinitionBase
{
	...
	public void CollectInitializationCallbacks()
	{
		...
		while (type != typeof(object))
		{
			...
			foreach (MethodInfo methodInfo in methods)
			{
				...
				{
					if (methodInfo.GetCustomAttributes(typeof(LoadInitializationCallback)).ToArray().Length != 0 && !_initializationCallbacks.Contains(methodInfo))
					{
						_initializationCallbacks.Insert(0, methodInfo);
					}
					if (methodInfo.GetCustomAttributes(typeof(LateLoadInitializationCallback)).ToArray().Length != 0 && !_lateInitializationCallbacks.Contains(methodInfo))
					{
	...
	public void CollectProperties()
	{
		...
		{
			Attribute[] array = propertyInfo.GetCustomAttributes(typeof(SaveablePropertyAttribute)).ToArray();
			if (array.Length != 0)
			{
	...
	public void CollectFields()
	{
		...
		foreach (FieldInfo fieldInfo in array)
		{
			Attribute[] array2 = fieldInfo.GetCustomAttributes(typeof(SaveableFieldAttribute)).ToArray();
			if (array2.Length != 0)
			{

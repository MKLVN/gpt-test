	...
	public static List<Type> CheckSaveableTypes()
	{
		...
		for (int i = 0; i < assemblies.Length; i++)
		{
			foreach (Type item in assemblies[i].GetTypesSafe())
			{
				...
				{
					Attribute[] array = fieldInfo.GetCustomAttributes(typeof(SaveableFieldAttribute)).ToArray();
					if (array.Length != 0)
					{
				...
				{
					Attribute[] array3 = propertyInfo.GetCustomAttributes(typeof(SaveablePropertyAttribute)).ToArray();
					if (array3.Length != 0)
					{

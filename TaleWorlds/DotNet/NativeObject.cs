	...
	static NativeObject()
	{
		...
		foreach (Assembly assembly in assemblies)
		{
			try
			{
				...
				{
					if (type.GetCustomAttributes(typeof(EngineClass), inherit: false).Length == 1)
					{
						list.Add(type);
					}
				}
		...
		{
			EngineClass engineClass = (EngineClass)item.GetCustomAttributes(typeof(EngineClass), inherit: false)[0];
			if (!item.IsAbstract)
			{

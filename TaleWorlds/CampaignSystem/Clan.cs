...
public sealed class Clan : MBObjectBase, IFaction
{
	...
	protected override void PreAfterLoad()
	{
		...
		if (MBSaveLoad.IsUpdatingGameVersion && MBSaveLoad.LastLoadedGameVersion < ApplicationVersion.FromString("v1.2.2") && Leader != null && Leader.Clan != this)
		{
	...
	private void SetKingdomInternal(Kingdom value)
	{
		...
		if (_kingdom != null)
		{
			ConsiderSiegesAndMapEvents(_kingdom);
			_kingdom.ConsiderSiegesAndMapEvents(_kingdom);
		}
	}

	public void ConsiderSiegesAndMapEvents(IFaction factionToConsiderAgainst)
	{
		foreach (WarPartyComponent item in _warPartyComponentsCache.ToList())
		{
			item.MobileParty.ConsiderMapEventsAndSiegesInternal(factionToConsiderAgainst);
		}
		foreach (Town fief in Fiefs)
		{

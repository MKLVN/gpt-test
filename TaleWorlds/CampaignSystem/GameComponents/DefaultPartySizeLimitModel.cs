...
public class DefaultPartySizeLimitModel : PartySizeLimitModel
{
	...
	private ExplainedNumber CalculateMobilePartyMemberSizeLimit(MobileParty party, bool includeDescriptions = false)
	{
		...
		if (party.LeaderHero != null && party.LeaderHero.Clan != null && !party.IsCaravan)
		{
			...
			{
				int num = 0;
				foreach (Settlement settlement in party.ActualClan.Settlements)
				{
			...
			if (party.LeaderHero.Clan.Leader == party.LeaderHero)
			{
				...
				{
					result.Add(party.LeaderHero.Clan.Tier * 25, _clanTierText);
				}
			}
			else if (party.LeaderHero.Clan.Tier > 0)
			{
				result.Add(party.LeaderHero.Clan.Tier * 15, _clanTierText);
			}
			result.Add(party.EffectiveQuartermaster.GetSkillValue(DefaultSkills.Steward) / 4, _quarterMasterText);
			AddMobilePartyLeaderPartySizePerkEffects(party, ref result);
			AddUltimateLeaderPerkEffectForPartySize(party, ref result);
	...
	{
		ExplainedNumber explainedNumber = new ExplainedNumber(20f, includeDescriptions: false, _baseSizeText);
		if (leaderHero != null && leaderHero.Clan != null)
		{
			...
			{
				explainedNumber.Add(20f, _factionLeaderText);
			}
			if (leaderHero.GetPerkValue(DefaultPerks.OneHanded.Prestige))
			{
				explainedNumber.Add(DefaultPerks.OneHanded.Prestige.SecondaryBonus, DefaultPerks.OneHanded.Prestige.Name);
			}
			if (leaderHero.GetPerkValue(DefaultPerks.TwoHanded.Hope))
			{
				explainedNumber.Add(DefaultPerks.TwoHanded.Hope.SecondaryBonus, DefaultPerks.TwoHanded.Hope.Name);
			}
			if (leaderHero.GetPerkValue(DefaultPerks.Athletics.ImposingStature))
			{
				explainedNumber.Add(DefaultPerks.Athletics.ImposingStature.SecondaryBonus, DefaultPerks.Athletics.ImposingStature.Name);
			}
			...
			{
				explainedNumber.Add(DefaultPerks.Tactics.HordeLeader.PrimaryBonus, DefaultPerks.Tactics.HordeLeader.Name);
			}
			if (leaderHero.GetPerkValue(DefaultPerks.Scouting.MountedScouts))
			{
				explainedNumber.Add(DefaultPerks.Scouting.MountedScouts.SecondaryBonus, DefaultPerks.Scouting.MountedScouts.Name);
			}
			if (leaderHero.GetPerkValue(DefaultPerks.Leadership.Authority))
			{
				explainedNumber.Add(DefaultPerks.Leadership.Authority.SecondaryBonus, DefaultPerks.Leadership.Authority.Name);
			}
			if (actualClan != null && actualClan.Leader?.GetPerkValue(DefaultPerks.Leadership.LeaderOfMasses) == true)
			{
				int num = 0;
				...
				if (num2 > 0f)
				{
					explainedNumber.Add(num2, DefaultPerks.Leadership.LeaderOfMasses.Name);
				}
			}
			if (leaderHero.Clan.Leader == leaderHero)
			{
				...
				{
					explainedNumber.Add(40f, DefaultPolicies.NobleRetinues.Name);
				}
				...
				{
					explainedNumber.Add(60f, DefaultPolicies.RoyalGuard.Name);
				}
				if (leaderHero.Clan.Tier > 0)
				{
					explainedNumber.Add(leaderHero.Clan.Tier * 25, _clanTierText);
				}
			}
			else if (leaderHero.Clan.Tier > 0)
			{
				explainedNumber.Add(leaderHero.Clan.Tier * 15, _clanTierText);
			}
		}

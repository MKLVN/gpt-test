	...
	public override ExplainedNumber GetDailyHealingForRegulars(MobileParty party, bool includeDescriptions = false)
	{
		...
		if (party.Party.IsStarving || (party.IsGarrison && party.CurrentSettlement.IsStarving))
		{
			...
			else
			{
				...
				bonuses.Add((float)(-totalRegulars) * 0.25f, _starvingText);
			}
		}
		else
		{
			bonuses = new ExplainedNumber(5f, includeDescriptions);
			if (party.IsGarrison)
			{
				if (party.CurrentSettlement.IsTown)
				{
					SkillHelper.AddSkillBonusForTown(DefaultSkills.Medicine, DefaultSkillEffects.GovernorHealingRateBonus, party.CurrentSettlement.Town, ref bonuses);
				}
			}
			else
			{
				SkillHelper.AddSkillBonusForParty(DefaultSkills.Medicine, DefaultSkillEffects.HealingRateBonusForRegulars, party, ref bonuses);
			}
			if (!party.IsGarrison && !party.IsMilitia)
			{
				if (!party.IsMoving)
				{
					PerkHelper.AddPerkBonusForParty(DefaultPerks.Medicine.TriageTent, party, isPrimaryBonus: true, ref bonuses);
					PerkHelper.AddPerkBonusForParty(DefaultPerks.Athletics.AGoodDaysRest, party, isPrimaryBonus: true, ref bonuses);
				}
				else
				{
					PerkHelper.AddPerkBonusForParty(DefaultPerks.Medicine.WalkItOff, party, isPrimaryBonus: true, ref bonuses);
					PerkHelper.AddPerkBonusForParty(DefaultPerks.Athletics.WalkItOff, party, isPrimaryBonus: true, ref bonuses);
				}
			}
			if (party.Morale >= Campaign.Current.Models.PartyMoraleModel.HighMoraleValue)
			{
				PerkHelper.AddPerkBonusForParty(DefaultPerks.Medicine.BestMedicine, party, isPrimaryBonus: true, ref bonuses);
			}
			if (party.CurrentSettlement != null && !party.CurrentSettlement.IsHideout)
			{
				if (party.CurrentSettlement.IsFortification)
				{
					bonuses.Add(10f, _settlementText);
				}
				if (party.SiegeEvent == null && !party.CurrentSettlement.IsUnderSiege && !party.CurrentSettlement.IsRaided && !party.CurrentSettlement.IsUnderRaid)
				{
					if (party.CurrentSettlement.IsTown)
					{
						PerkHelper.AddPerkBonusForParty(DefaultPerks.Medicine.PristineStreets, party, isPrimaryBonus: false, ref bonuses);
					}
					PerkHelper.AddPerkBonusForParty(DefaultPerks.Medicine.GoodLogdings, party, isPrimaryBonus: true, ref bonuses);
				}
			}
			...
			{
				PerkHelper.AddPerkBonusForParty(DefaultPerks.Medicine.BushDoctor, party, isPrimaryBonus: false, ref bonuses);
			}
			if (party.Army != null)
			{
				PerkHelper.AddPerkBonusForParty(DefaultPerks.Scouting.Rearguard, party, isPrimaryBonus: true, ref bonuses);
			}
			if (party.ItemRoster.FoodVariety > 0 && party.HasPerk(DefaultPerks.Medicine.PerfectHealth))
			{
				bonuses.AddFactor((float)party.ItemRoster.FoodVariety * DefaultPerks.Medicine.PerfectHealth.PrimaryBonus, DefaultPerks.Medicine.PerfectHealth.Name);
			}
			if (party.HasPerk(DefaultPerks.Medicine.HelpingHands))
			{
				...
				bonuses.AddFactor(value, DefaultPerks.Medicine.HelpingHands.Name);
			}
		}
		return bonuses;
	}

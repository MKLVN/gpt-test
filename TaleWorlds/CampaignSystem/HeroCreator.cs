...
public static class HeroCreator
{
	...
	private static Hero CreateNewHero(CharacterObject template, int age = -1)
	{
		...
		{
			newCharacter.HeroObject.HeroDeveloper.DeriveSkillsFromTraits();
		}
	...
	{
		Debug.SilentAssert(mother.CharacterObject.Race == father.CharacterObject.Race, "", getDump: false, "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\HeroCreator.cs", "DeliverOffSpring", 482);
		...
		NameGenerator.Current.GenerateHeroNameAndHeroFullName(hero, out var firstName, out var fullName, useDeterministicValues: false);
		hero.SetName(fullName, firstName);
		hero.HeroDeveloper.DeriveSkillsFromTraits(isByNaturalGrowth: true);

...
public static class CampaignCheats
{
	...
	public static string DeclarePeace(List<string> strings)
	{
		...
		if (faction == null)
		{
			return string.Concat("Faction is not found: ", faction, "\n", text);
		}
		return string.Concat("Faction is not found: ", faction2, "\n", text);
	}
	...
	public static string AddCompanion(List<string> strings)
	{
		...
		int changeAmount = TaleWorlds.Library.MathF.Min(hero.Level / 25 + 1, 10);
		foreach (SkillObject item in Skills.All)
		{
			hero.HeroDeveloper.AddFocus(item, changeAmount, checkUnspentFocusPoints: false);
			hero.HeroDeveloper.SetInitialSkillLevel(item, hero.Level);
		}
		hero.HeroDeveloper.UnspentFocusPoints = 0;
	...
	public static string CreateRandomClan(List<string> strings)
	{
		...
		hero.HeroDeveloper.DeriveSkillsFromTraits();
		hero.ChangeState(Hero.CharacterStates.Active);
		clan.SetLeader(hero);
	...
	public static string AddAttributePointsCheat(List<string> strings)
	{
		...
		else
		{
			...
			{
				mainHero = GetHero(separatedNames[1]);
				if (mainHero != null)
				{
					mainHero.HeroDeveloper.UnspentAttributePoints = MBMath.ClampInt(mainHero.HeroDeveloper.UnspentAttributePoints + result2, 0, int.MaxValue);
					flag = true;
					num = result2;
				}
			}
			else
			{
				...
				if (mainHero != null)
				{
					mainHero.HeroDeveloper.UnspentAttributePoints = MBMath.ClampInt(mainHero.HeroDeveloper.UnspentAttributePoints + 1, 0, int.MaxValue);
					flag = true;
				}
			}
		}

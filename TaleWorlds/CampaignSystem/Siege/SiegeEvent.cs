...
public class SiegeEvent
{
	...
	public class SiegeEngineConstructionProgress
	{
		...
		{
			return ((SiegeEngineConstructionProgress)o).IsActive;
		}
	...
	public class SiegeEnginesContainer
	{
		...
		public void DeploySiegeEngineAtIndex(SiegeEngineConstructionProgress siegeEngine, int index)
		{
			...
			{
				siegeEngineConstructionProgress.Activate(isActive: false);
			...
			siegeEngine.Activate(isActive: true);
	...
	public bool CanPartyJoinSide(PartyBase party, BattleSideEnum side)
	{
		...
		if (!FactionManager.IsAtWarAgainstFaction(GetSiegeEventSide(side).GetInvolvedPartiesForEventType().First().MapFaction, mapFaction))
		{
			return FactionManager.IsAtWarAgainstFaction(GetSiegeEventSide(side2).GetInvolvedPartiesForEventType().First().MapFaction, mapFaction);
		}
		return false;
	}
	...
	public Dictionary<SiegeEngineType, int> GetPreparedSiegeEnginesAsDictionary(ISiegeEventSide siegeEventSide)
	{
		...
		{
			if (item.IsConstructed && item.Hitpoints > 0f && item.SiegeEngine != DefaultSiegeEngineTypes.Preparations)
			{
	...
	public void ConstructionTick(ISiegeEventSide siegeEventSide)
	{
		...
		else
		{
			...
			{
				if (deployedSiegeEngine.IsActive && !deployedSiegeEngine.IsConstructed && !deployedSiegeEngine.IsBeingRedeployed)
				{
		...
		{
			if (deployedSiegeEngine2.IsActive && deployedSiegeEngine2.IsConstructed && deployedSiegeEngine2.IsBeingRedeployed)
			{
	...
	public void BombardTick(ISiegeEventSide siegeEventSide)
	{
		...
		foreach (SiegeEngineConstructionProgress deployedSiegeEngine in siegeEventSide.SiegeEngines.DeployedSiegeEngines)
		{
			if (!deployedSiegeEngine.IsConstructed || deployedSiegeEngine.IsBeingRedeployed || !deployedSiegeEngine.IsActive)
			{
				continue;
			}

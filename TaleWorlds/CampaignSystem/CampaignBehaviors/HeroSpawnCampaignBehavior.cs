...
public class HeroSpawnCampaignBehavior : CampaignBehaviorBase
{
	...
	private static Hero CreateMinorFactionHeroFromTemplate(CharacterObject template, Clan faction)
	{
		...
		hero.HeroDeveloper.DeriveSkillsFromTraits(isByNaturalGrowth: false, template);
		return hero;
	}
	...
	private static void SpawnMinorFactionHeroes(Clan clan, bool firstTime)
	{
		...
		{
			Debug.FailedAssert($"{clan.Name} templates are empty!", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\CampaignBehaviors\\HeroSpawnCampaignBehavior.cs", "SpawnMinorFactionHeroes", 638);
			return;
		}
		for (int j = 0; j < num; j++)
		{

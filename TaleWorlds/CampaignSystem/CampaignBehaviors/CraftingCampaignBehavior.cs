...
public class CraftingCampaignBehavior : CampaignBehaviorBase, ICraftingCampaignBehavior, ICampaignBehavior
{
	...
	private void HourlyTick()
	{
		foreach (KeyValuePair<Hero, HeroCraftingRecord> heroCraftingRecord in _heroCraftingRecords)
		{
			...
			if (heroCraftingRecord.Value.CraftingStamina < maxHeroCraftingStamina)
			{
				...
				if (key.PartyBelongedTo?.CurrentSettlement != null)
				{
	...
	public void InitializeCraftingElements()
	{
		...
		foreach (KeyValuePair<Town, CraftingOrderSlots> craftingOrder3 in CraftingOrders)
		{
			CraftingOrder[] slots = craftingOrder3.Value.Slots;
			for (int j = 0; j < slots.Length; j++)
			{
				slots[j]?.InitializeCraftingOrderOnLoad();
			}
			foreach (CraftingOrder customOrder2 in craftingOrder3.Value.CustomOrders)
			{
				customOrder2.InitializeCraftingOrderOnLoad();
			}
		}
	}
	...
	public ItemObject CreateCraftedWeaponInFreeBuildMode(Hero hero, WeaponDesign weaponDesign, ItemModifier weaponModifier = null)
	{
		...
		if (GameStateManager.Current.ActiveState is CraftingState craftingState)
		{
			...
			TextObject textObject = GameTexts.FindText("crafting_added_to_inventory");
			textObject.SetCharacterProperties("PLAYER", Hero.MainHero.CharacterObject);
			textObject.SetTextVariable("ITEM_NAME", itemObject.Name);
			MBInformationManager.AddQuickInformation(textObject);
	...
	public ItemObject CreateCraftedWeaponInCraftingOrderMode(Hero crafterHero, CraftingOrder craftingOrder, WeaponDesign weaponDesign)
	{
		...
		if (GameStateManager.Current.ActiveState is CraftingState craftingState)
		{
			...
			float xpAmount = craftingOrder.GetOrderExperience(itemObject) + (float)Campaign.Current.Models.SmithingModel.GetSkillXpForSmithingInCraftingOrderMode(itemObject);
	...
	public void GetOrderResult(CraftingOrder craftingOrder, ItemObject craftedItem, out bool isSucceed, out TextObject orderRemark, out TextObject orderResult, out int finalReward)
	{
		...
		craftingOrder.CheckForBonusesAndPenalties(craftedItem, out var craftedStatsSum, out var requiredStatsSum, out var thrustDamageCheck, out var swingDamageCheck);

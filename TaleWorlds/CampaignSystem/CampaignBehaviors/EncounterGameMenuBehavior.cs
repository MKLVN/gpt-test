...
public class EncounterGameMenuBehavior : CampaignBehaviorBase
{
	...
	private void AddGameMenus(CampaignGameStarter gameSystemInitializer)
	{
		...
		gameSystemInitializer.AddGameMenu("hostile_action_end_by_leave_kingdom_as_mercenary", "{=lBHm1JlL}Your kingdom has ended your mercenary contract.", null);
		gameSystemInitializer.AddGameMenuOption("hostile_action_end_by_leave_kingdom_as_mercenary", "hostile_action_end_by_leave_kingdom_end", "{=2YYRyrOO}Leave...", game_menu_siege_attacker_defeated_leave_on_condition, delegate
		{
			PlayerEncounter.Finish();
		}, isLeave: true);
		gameSystemInitializer.AddGameMenu("hostile_action_end_by_peace", "{=1rbg3Hz2}The {HOSTILE_FACTION} and {SETTLEMENT_FACTION} are no longer enemies.", game_menu_hostile_action_end_by_peace_on_init);
		...
		gameSystemInitializer.AddGameMenuOption("fortification_crime_rating", "fortification_crime_rating_continue", "{=WVkc4UgX}Continue.", game_menu_fortification_high_crime_rating_continue_on_condition, game_menu_fortification_high_crime_rating_continue_on_consequence);
	...
	{
		PlayerEncounter.Current?.ResetPlayerEncounterInterruptedByPeace();
	...
	private bool game_menu_encounter_order_attack_on_condition(MenuCallbackArgs args)
	{
		...
		if (playerMapEvent != null)
		{
			...
			{
				num += item.Party.MemberRoster.Sum((TroopRosterElement x) => x.Character.IsHero ? ((x.Character != CharacterObject.PlayerCharacter) ? ((!x.Character.HeroObject.IsWounded) ? 1 : 0) : 0) : (x.Number - x.WoundedNumber));
			}
			if (playerMapEvent.HasTroopsOnBothSides() && playerMapEvent.GetLeaderParty(PartyBase.MainParty.OpponentSide) != null && num > 0)
			{
				if (MobileParty.MainParty.MemberRoster.Sum((TroopRosterElement x) => x.Character.IsHero ? ((x.Character != CharacterObject.PlayerCharacter) ? ((!x.Character.HeroObject.IsWounded) ? 1 : 0) : 0) : (x.Number - x.WoundedNumber)) > 0)
				{

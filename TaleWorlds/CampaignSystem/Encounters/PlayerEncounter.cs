...
public class PlayerEncounter
{
	...
	public bool IsPlayerEncounterInterruptedByPeace => _isPlayerEncounterInterruptedByPeace;
	...
	public static void Finish(bool forcePlayerOutFromSettlement = true)
	{
		...
		return;
		IL_010e:
		if (Current._isPlayerEncounterInterruptedByPeace)
		{
			GameMenu.ActivateGameMenu("hostile_action_end_by_peace");
			Current._isPlayerEncounterInterruptedByPeace = false;
		}
		...
		while (!_stateHandled)
		{
			...
			if (!_stateHandled)
			{
				...
				case PlayerEncounterState.End:
					...
					break;
				default:
					Debug.FailedAssert("[DEBUG]Invalid map event state: " + _mapEventState, "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Encounters\\PlayerEncounter.cs", "UpdateInternal", 1142);
					break;
				}
			}
		}
	...
	{
		Current._isPlayerEncounterInterruptedByPeace = true;
		if (_mapEvent != null)
		{
			_mapEvent.DiplomaticallyFinished = true;
		}
	}
	...
	private void DoFreeHeroes()
	{
		...
		if (_freedHeroes.Count > 0)
		{
			TroopRosterElement troopRosterElement = _freedHeroes[_freedHeroes.Count - 1];
			...
			Campaign.Current.ConversationManager.ConversationEndOneShot += delegate
			{
				_freedHeroes.RemoveRange(_freedHeroes.Count - 1, 1);
			};
			_stateHandled = true;
		}
		else
	...
	{
		LocatableSearchData<MobileParty> data = MobileParty.StartFindingLocatablesAroundPosition(MobileParty.MainParty.Position2D, 4f);

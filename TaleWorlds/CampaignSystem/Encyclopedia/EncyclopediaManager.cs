...
public class EncyclopediaManager
{
	...
	public void CreateEncyclopediaPages()
	{
		...
		foreach (Type item2 in list)
		{
			...
			object[] customAttributes = item2.GetCustomAttributes(typeof(OverrideEncyclopediaModel), inherit: false);
			for (int i = 0; i < customAttributes.Length; i++)
			{
				if (customAttributes[i] is OverrideEncyclopediaModel overrideEncyclopediaModel)
				{
		...
		foreach (Type item3 in list)
		{
			...
			object[] customAttributes = item3.GetCustomAttributes(typeof(EncyclopediaModel), inherit: false);
			for (int i = 0; i < customAttributes.Length; i++)
			{
				if (!(customAttributes[i] is EncyclopediaModel encyclopediaModel))
				{
					continue;
				}

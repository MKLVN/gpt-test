...
public static class GainKingdomInfluenceAction
{
	...
	private static void ApplyInternal(Hero hero, MobileParty party, float gainedInfluence, InfluenceGainingReason detail)
	{
		...
		if (clan.Kingdom != null)
		{
			...
			ExplainedNumber bonuses = new ExplainedNumber(gainedInfluence);
			if (detail == InfluenceGainingReason.Battle && gainedInfluence > 0f)
			{
				PerkHelper.AddPerkBonusForParty(DefaultPerks.Tactics.PreBattleManeuvers, mobileParty, isPrimaryBonus: true, ref bonuses);
			}
			if (detail == InfluenceGainingReason.CaptureSettlement && (hero != null || mobileParty.LeaderHero != null))
			{
				...
				PerkHelper.AddPerkBonusForCharacter(DefaultPerks.Tactics.Besieged, hero2.CharacterObject, isPrimaryBonus: false, ref bonuses);
			}
			ChangeClanInfluenceAction.Apply(clan, bonuses.ResultNumber);

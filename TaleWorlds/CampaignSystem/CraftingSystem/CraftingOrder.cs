...
public class CraftingOrder : ITrackableCampaignObject, ITrackableBase
{
	...
	public float GetOrderExperience(ItemObject craftedItem)
	{
		...
		CheckForBonusesAndPenalties(craftedItem, out var craftedStatsSum, out var requiredStatsSum, out var thrustDamageCheck, out var swingDamageCheck);
	...
	public void CheckForBonusesAndPenalties(ItemObject craftedItem, out float craftedStatsSum, out float requiredStatsSum, out bool thrustDamageCheck, out bool swingDamageCheck)
	{
		...
		List<CraftingStatData> statDataForItem = GetStatDataForItem(PreCraftedWeaponDesignItem, out weapon);
		list = GetStatDataForItem(craftedItem, out var weapon2);
		...
		requiredStatsSum = 0f;
		craftedStatsSum = 0f;
		foreach (CraftingStatData orderStat in statDataForItem)
		{
			if (orderStat.IsValid)
			{
				CraftingStatData craftingStatData = list.FirstOrDefault((CraftingStatData x) => x.Type == orderStat.Type);

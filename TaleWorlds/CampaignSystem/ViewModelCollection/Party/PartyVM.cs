...
public class PartyVM : ViewModel, IPartyScreenLogicHandler, PartyScreenPrisonHandler, IPartyScreenTroopHandler
{
	...
	private void InitializeTroopLists()
	{
		...
		CurrentCharacter = ((MainPartyTroops.Count > 0) ? MainPartyTroops[0] : OtherPartyTroops[0]);
	...
	private void InitializePartyList(MBBindingList<PartyCharacterVM> partyList, TroopRoster currentTroopRoster, PartyScreenLogic.TroopType type, int side)
	{
		...
		for (int i = 0; i < currentTroopRoster.Count; i++)
		{
			...
			PartyCharacterVM partyCharacterVM = new PartyCharacterVM(PartyScreenLogic, this, currentTroopRoster, i, type, (PartyScreenLogic.PartyRosterSide)side, PartyScreenLogic.IsTroopTransferable(type, elementCopyAtIndex.Character, side));
	...
	{
		return FindCharacterVM(character, fromSide, isPrisoner).WoundedCount;
	}
	...
	private void RecruitTroop(PartyScreenLogic.PartyCommand command)
	{
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("Players can only recruit prisoners", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem.ViewModelCollection\\Party\\PartyVM.cs", "RecruitTroop", 1025);
		}
		if (num < 0)
		{
	...
	private void ExecuteTroop(PartyScreenLogic.PartyCommand command)
	{
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("Players can only execute prisoners", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem.ViewModelCollection\\Party\\PartyVM.cs", "ExecuteTroop", 1065);
		}
		if (num < 0)
		{
			partyCharacterVMList2.Remove(CurrentCharacter);
		...
		{
			TaleWorlds.Library.Debug.FailedAssert("The prisoner should have been removed from the prisoner roster after execution", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem.ViewModelCollection\\Party\\PartyVM.cs", "ExecuteTroop", 1076);
		}
		RefreshTopInformation();
		RefreshPartyInformation();
	}
	...
	public void ExecuteTalk()
	{
		if (PartyScreenLogic.IsThereAnyChanges())
		{
			...
			{
				ExecuteRemoveZeroCounts();
				InformationManager.ShowInquiry(new InquiryData(new TextObject("{=*}Apply Changes?").ToString(), new TextObject("{=*}You need to confirm your changes in order to engage in a conversation.").ToString(), isAffirmativeOptionShown: true, isNegativeOptionShown: true, GameTexts.FindText("str_yes").ToString(), GameTexts.FindText("str_no").ToString(), delegate
				{
					...
					{
						InformationManager.ShowInquiry(new InquiryData(new TextObject("{=*}Failed to Apply Changes").ToString(), new TextObject("{=*}Could not apply changes.").ToString(), isAffirmativeOptionShown: true, isNegativeOptionShown: false, GameTexts.FindText("str_ok").ToString(), string.Empty, null, null));
					}
				}, null));
			}
			else
			{
				InformationManager.ShowInquiry(new InquiryData(new TextObject("{=*}Reset Changes?").ToString(), new TextObject("{=*}Cannot apply changes. You need reset your changes in order to engage in a conversation.").ToString(), isAffirmativeOptionShown: true, isNegativeOptionShown: true, GameTexts.FindText("str_yes").ToString(), GameTexts.FindText("str_no").ToString(), delegate
				{
					ExecuteReset();
					ExecuteOpenConversation();
				}, null));
	...
	private void EnsureLogicRostersAreInSyncWithVMLists()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			{
				TaleWorlds.Library.Debug.FailedAssert("Logic and VM list counts do not match", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem.ViewModelCollection\\Party\\PartyVM.cs", "EnsureLogicRostersAreInSyncWithVMLists", 1721);
				continue;
			}
			for (int j = 0; j < list[i].Count; j++)
			{
				...
				{
					TaleWorlds.Library.Debug.FailedAssert("Logic and VM rosters do not match", "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem.ViewModelCollection\\Party\\PartyVM.cs", "EnsureLogicRostersAreInSyncWithVMLists", 1729);
					return;
				}
			}
		}

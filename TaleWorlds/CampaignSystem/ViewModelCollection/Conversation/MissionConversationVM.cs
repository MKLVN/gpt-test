...
public class MissionConversationVM : ViewModel
{
	...
	public MissionConversationVM(Func<string> getContinueInputText, bool isLinksDisabled = false)
	{
		...
		if (IsAggressive)
		{
			...
			foreach (MobileParty item2 in partiesToJoinPlayerSide)
			{
	...
	public void Refresh()
	{
		...
		if (num > 0 && !_isCurrentlyPlayerSpeaking)
		{
			for (int i = 0; i < num; i++)
			{
				AnswerList.Add(new ConversationItemVM(OnSelectOption, OnReadyToContinue, ExecuteSetCurrentAnswer, i, _conversationManager.CurOptions[i]));
			}
		}

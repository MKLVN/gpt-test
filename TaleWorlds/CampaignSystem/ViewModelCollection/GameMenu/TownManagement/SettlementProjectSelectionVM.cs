...
public class SettlementProjectSelectionVM : ViewModel
{
	...
	public void Refresh()
	{
		...
		foreach (Building item in Settlement.CurrentSettlement.Town.Buildings.Where((Building x) => x.BuildingType.BuildingLocation != BuildingLocation.Daily))
		{
			SettlementBuildingProjectVM settlementBuildingProjectVM = new SettlementBuildingProjectVM(OnCurrentProjectSelection, OnCurrentProjectSet, OnResetCurrentProject, item);
		...
		{
			foreach (Building item2 in Settlement.CurrentSettlement.Town.Buildings.Where((Building x) => x.BuildingType.BuildingLocation == BuildingLocation.Daily))
			{
				SettlementDailyProjectVM settlementDailyProjectVM = new SettlementDailyProjectVM(OnCurrentProjectSelection, OnCurrentProjectSet, OnResetCurrentProject, item2);
		...
		foreach (Building item3 in _town.BuildingsInProgress)
		{
			LocalDevelopmentList.Add(item3);
		}

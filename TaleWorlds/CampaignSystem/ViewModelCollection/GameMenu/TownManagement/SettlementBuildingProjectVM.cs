...
public class SettlementBuildingProjectVM : SettlementProjectVM
{
	...
	public override void RefreshProductionText()
	{
		...
		if (DevelopmentQueueIndex == 0)
		{
			...
			if (daysToComplete != -1)
			{
				empty = new TextObject("{=c5eYzHaM}{DAYS} {?DAY_IS_PLURAL}Days{?}Day{\\?} ({PERCENTAGE}%)");
				empty.SetTextVariable("DAYS", daysToComplete);
				GameTexts.SetVariable("DAY_IS_PLURAL", (daysToComplete > 1) ? 1 : 0);
			}
			else
			{
				empty = new TextObject("{=0TauthlH}Never ({PERCENTAGE}%)");
			}
			empty.SetTextVariable("PERCENTAGE", (int)(BuildingHelper.GetProgressOfBuilding(base.Building, Settlement.CurrentSettlement.Town) * 100f));
			GameTexts.SetVariable("RIGHT", empty);

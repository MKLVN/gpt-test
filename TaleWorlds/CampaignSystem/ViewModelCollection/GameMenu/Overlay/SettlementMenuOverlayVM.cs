...
public class SettlementMenuOverlayVM : GameMenuOverlay
{
	...
	public SettlementMenuOverlayVM(GameOverlays.MenuOverlayType type)
	{
		...
		{
			RemainingFoodHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownFoodTooltip(Settlement.CurrentSettlement.Town));
			LoyaltyHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownLoyaltyTooltip(Settlement.CurrentSettlement.Town));
			MilitasHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownMilitiaTooltip(Settlement.CurrentSettlement.Town));
			ProsperityHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownProsperityTooltip(Settlement.CurrentSettlement.Town));
			WallsHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownWallsTooltip(Settlement.CurrentSettlement.Town));
			GarrisonHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownGarrisonTooltip(Settlement.CurrentSettlement.Town));
			SecurityHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetTownSecurityTooltip(Settlement.CurrentSettlement.Town));
		}
		else
		{
			MilitasHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetVillageMilitiaTooltip(Settlement.CurrentSettlement.Village));
			LoyaltyHint = new BasicTooltipViewModel();
			WallsHint = new BasicTooltipViewModel();
			ProsperityHint = new BasicTooltipViewModel(() => CampaignUIHelper.GetVillageProsperityTooltip(Settlement.CurrentSettlement.Village));
		}
		if (Settlement.CurrentSettlement.IsFortification)
		{
			_settlementComponent = Settlement.CurrentSettlement.SettlementComponent;
		}
		else if (Settlement.CurrentSettlement.IsVillage)
		{
			_settlementComponent = Settlement.CurrentSettlement.Village;
		}
		else
		{
			_settlementComponent = Settlement.CurrentSettlement.SettlementComponent;
	...
	{
		InformationManager.ShowTooltip(typeof(Settlement), Settlement.CurrentSettlement, true);
	}
	...
	private void InitCharacterList()
	{
		if (_type == GameOverlays.MenuOverlayType.SettlementWithCharacters || _type == GameOverlays.MenuOverlayType.SettlementWithBoth)
		{
			...
			{
				foreach (LocationCharacter character in menuLocation.GetCharacterList())
				{
					if (WillBeListed(character) && !heroesInSettlement.ContainsKey(character.Character.HeroObject))
					{
						heroesInSettlement.Add(character.Character.HeroObject, character.UseCivilianEquipment);
					}
				}
			}
			UpdateList(CharacterList, heroesInSettlement.Keys, new CharacterComparer(), (GameMenuPartyItemVM x) => x?.Character?.HeroObject, (Hero x) => heroesInSettlement.ContainsKey(x), (Hero x) => new GameMenuPartyItemVM(ExecuteOnSetAsActiveContextMenuItem, x.CharacterObject, heroesInSettlement[x]));
		}
		else
		{
			CharacterList.Clear();
	...
	private void InitPartyList()
	{
		if (_type == GameOverlays.MenuOverlayType.SettlementWithBoth || _type == GameOverlays.MenuOverlayType.SettlementWithParties)
		{
			...
			UpdateList(PartyList, partiesInSettlement, new PartyComparer(), (GameMenuPartyItemVM x) => x?.Party?.MobileParty, (MobileParty x) => partiesInSettlement.Contains(x), (MobileParty x) => new GameMenuPartyItemVM(ExecuteOnSetAsActiveContextMenuItem, x.Party, canShowQuest: false));
		}
		else
		{
			PartyList.Clear();
	...
	private void UpdateSettlementOwnerBanner()
	{
		...
		else if (Settlement.CurrentSettlement.OwnerClan?.Banner != null)
		{
	...
	{
		if (Settlement.CurrentSettlement != null && (settlement == Settlement.CurrentSettlement || (Settlement.CurrentSettlement.IsVillage && settlement.BoundVillages.Contains(Settlement.CurrentSettlement.Village))))
		{
			UpdateSettlementOwnerBanner();
		}
	}

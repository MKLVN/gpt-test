		...
		else
		{
			...
			if (!settlement.IsHideout && (flag2 || flag))
			{
				...
				if (list.Count > 0)
				{
					...
					{
						propertyBasedTooltipVM.AddProperty(item.Name.ToString(), CampaignUIHelper.GetPartyNameplateText(item));
					}
					propertyBasedTooltipVM.AddProperty(string.Empty, string.Empty, -1);
				}
				if (list2.Count > 0)
				{
					...
					{
						propertyBasedTooltipVM.AddProperty(item2.Name.ToString(), CampaignUIHelper.GetPartyNameplateText(item2));
					}
				}
			}
			else

...
public class WeaponDesignVM : ViewModel
{
	...
	private void UpdateResultPropertyList()
	{
		...
		if (IsInOrderMode)
		{
			WeaponComponentData orderWeapon = ActiveCraftingOrder.CraftingOrder.GetStatWeapon();
			usageIndex = _crafting.GetCurrentCraftedItemObject().Weapons.FindIndex((WeaponComponentData x) => x.WeaponDescriptionId == orderWeapon.WeaponDescriptionId);
		}
		...
		foreach (CraftingListPropertyItem primaryProperty in PrimaryPropertyList)
		{
			...
			if (IsInOrderMode)
			{
				DesignResultPropertyList.Add(new WeaponDesignResultPropertyItemVM(primaryProperty.Description, primaryProperty.PropertyValue, primaryProperty.TargetValue, changeAmount, showFloatingPoint, primaryProperty.IsExceedingBeneficial));
			}
			else
			{
				DesignResultPropertyList.Add(new WeaponDesignResultPropertyItemVM(primaryProperty.Description, primaryProperty.PropertyValue, changeAmount, showFloatingPoint));
			}
		}
	}
	...
	{
		CraftingResultPopup?.OnFinalize();
		CraftingResultPopup = new WeaponDesignResultPopupVM(ExecuteFinalizeCrafting, _crafting, ActiveCraftingOrder?.CraftingOrder, WeaponFlagIconsList, CraftedItemObject, _designResultPropertyList, _itemName, _craftedItemVisual);
	}
	...
	public void ExecuteFinalizeCrafting()
	{
		...
		if (GameStateManager.Current.ActiveState is CraftingState)
		{
			...
			else
			{
				...
				ItemModifier currentItemModifier = _craftingBehavior.GetCurrentItemModifier();
				_craftingBehavior.CreateCraftedWeaponInFreeBuildMode(_getCurrentCraftingHero?.Invoke()?.Hero, _crafting.CurrentWeaponDesign, currentItemModifier);
	...
	{
		_tempItemObjectForVisual = _crafting.GetCurrentCraftedItemObject(forceReCreate: true);
		_tempItemObjectForVisual.StringId = _tempItemObjectForVisual.WeaponDesign.HashedCode;
		MBObjectManager.Instance.RegisterObject(_tempItemObjectForVisual);
		if (MBObjectManager.Instance.GetObject<ItemObject>(_tempItemObjectForVisual.StringId) == null)
		{
			MBObjectManager.Instance.RegisterObject(_tempItemObjectForVisual);
			return;
		}
		CraftedItemVisual.StringId = _tempItemObjectForVisual.StringId;
		IsWeaponCivilian = _tempItemObjectForVisual.IsCivilian;
	}

	public void AddCraftingModifier(TaleWorlds.Core.WeaponDesign weaponDesign, Hero hero)
	{
	...
	{
		if (_tempItemObjectForVisual != null)
		{
			MBObjectManager.Instance.UnregisterObject(_tempItemObjectForVisual);
		}
	}

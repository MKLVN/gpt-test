...
public class GameMenuCallbackManager
{
	...
	private void FillInitializationHandlerWith(Assembly assembly)
	{
		foreach (Type item in assembly.GetTypesSafe())
		{
			...
			{
				object[] customAttributes = methodInfo.GetCustomAttributes(typeof(GameMenuInitializationHandler), inherit: false);
				...
				for (int j = 0; j < array.Length; j++)
				{
					...
					GameMenuInitializationHandlerDelegate value = Delegate.CreateDelegate(typeof(GameMenuInitializationHandlerDelegate), methodInfo) as GameMenuInitializationHandlerDelegate;
	...
	private void FillEventHandlersWith(Assembly assembly)
	{
		foreach (Type item in assembly.GetTypesSafe())
		{
			...
			foreach (MethodInfo methodInfo in methods)
			{
				object[] customAttributes = methodInfo.GetCustomAttributes(typeof(GameMenuEventHandler), inherit: false);
				if (customAttributes == null || customAttributes.Length == 0)
				{
					continue;
				}
				...
				for (int j = 0; j < array.Length; j++)
				{
					...
					GameMenuEventHandlerDelegate value = Delegate.CreateDelegate(typeof(GameMenuEventHandlerDelegate), methodInfo) as GameMenuEventHandlerDelegate;

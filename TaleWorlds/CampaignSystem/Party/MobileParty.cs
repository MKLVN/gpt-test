...
public sealed class MobileParty : CampaignObjectBase, ILocatable<MobileParty>, IMapPoint, ITrackableCampaignObject, ITrackableBase, IMapEntity, IRandomOwner
{
	...
	{
		if (Army != null && Army.Kingdom != MapFaction)
		{
			Army = null;
		}
		if (CurrentSettlement != null)
		{
			...
			if ((mapFaction != null && mapFaction.IsAtWarWith(MapFaction) && DefaultBehavior == AiBehavior.RaidSettlement) || (IsMainParty && PlayerEncounter.Current != null && (PlayerEncounter.Current.ForceRaid || PlayerEncounter.Current.ForceSupplies || PlayerEncounter.Current.ForceVolunteers)))
			{
				return;
			}
		}
		if (Party.MapEventSide != null && !Party.MapEvent.IsFinalized)
		{
			if (!MapEvent.CanPartyJoinBattle(Party, Party.MapEventSide.MissionSide))
			{
			...
			if (Party == PartyBase.MainParty && PlayerEncounter.Current != null && PlayerEncounter.Battle != null && !PlayerEncounter.EncounteredParty.MapFaction.IsAtWarWith(MainParty.MapFaction) && PlayerEncounter.Current.IsPlayerEncounterInterruptedByPeace && Game.Current.GameStateManager.ActiveState is MapState)
			{
				PlayerEncounter.Finish();
			}
		}
		BattleSideEnum battleSideEnum = ((PlayerEncounter.Battle != null) ? PlayerEncounter.Battle.PlayerSide : BattleSideEnum.None);
		bool flag = PlayerEncounter.EncounterSettlement != null && PlayerEncounter.EncounterSettlement.SiegeEvent != null && PlayerEncounter.EncounterSettlement.SiegeEvent.IsPartyInvolved(Party);
		if (BesiegerCamp != null || flag)
		{
			Settlement obj = BesiegerCamp?.SiegeEvent.BesiegedSettlement ?? PlayerEncounter.EncounterSettlement;
			MobileParty mobileParty = BesiegerCamp?.LeaderParty ?? PlayerEncounter.EncounterSettlement?.SiegeEvent.BesiegerCamp.LeaderParty;
			IFaction mapFaction2 = obj.MapFaction;
			IFaction faction = mobileParty?.MapFaction;
			if (mapFaction2 == null || (!mapFaction2.IsAtWarWith(MapFaction) && mapFaction2 == factionToConsiderAgainst))
			{
				if (Party == PartyBase.MainParty && battleSideEnum == BattleSideEnum.None)
				{
					GameMenu.ActivateGameMenu("hostile_action_end_by_peace");
				}
				else if (PlayerEncounter.Current != null && (PlayerEncounter.EncounteredParty == Party || (PlayerEncounter.EncounterSettlement != null && PlayerEncounter.EncounterSettlement.SiegeEvent != null && PlayerEncounter.EncounterSettlement.SiegeEvent == SiegeEvent)) && mobileParty != null && mobileParty == this)
				{
					PlayerEncounter.Current.SetPlayerEncounterInterruptedByPeace();
				}
				if (Army == null || Army.LeaderParty == this)
				{
					BesiegerCamp = null;
					Ai.SetMoveModeHold();
					if (Party.MapEventSide != null && !Party.MapEvent.IsFieldBattle && !Party.MapEvent.IsSiegeAssault)
					{
						Party.MapEvent.FinishSiegeEventKeepBattle();
					}
				}
			}
			else if (faction == null || (faction.IsAtWarWith(MapFaction) && mapFaction2 == factionToConsiderAgainst))
			{
				BesiegerCamp = null;
				Ai.SetMoveModeHold();
				if (Party.MapEventSide != null)
			...
			else if (mapFaction2 != null && faction != null && MapFaction != mapFaction2 && MapFaction != faction && PlayerEncounter.EncounterSettlement != null && PlayerEncounter.EncounterSettlement.SiegeEvent != null && !PartyBase.MainParty.MapFaction.IsAtWarWith(PlayerEncounter.EncounterSettlement.SiegeEvent.BesiegerCamp.LeaderParty.MapFaction))
			{
				PlayerEncounter.Current.SetPlayerEncounterInterruptedByPeace();
			}
		}
		if (CurrentSettlement != null)
		{
			...
			if (mapFaction3 != null && mapFaction3 == factionToConsiderAgainst && mapFaction3.IsAtWarWith(MapFaction))
			{
				if (IsMainParty)
				{
					if (!GameStateManager.Current.ActiveState.IsMission && CurrentSettlement.IsFortification)
					{
				...
				else if (Army.LeaderParty == this)
				{
					if (!Army.Parties.Contains(MainParty))
					{
						Settlement currentSettlement2 = CurrentSettlement;
						LeaveSettlementAction.ApplyForParty(this);
		...
		if (PlayerEncounter.EncounteredBattle != null && !PlayerEncounter.EncounteredBattle.IsFinalized)
		{
			...
			{
				if (!encounteredBattle.GetLeaderParty(encounteredBattle.PlayerSide.GetOppositeSide()).MapFaction.IsAtWarWith(MainParty.MapFaction))
				{
					PlayerEncounter.Current.SetPlayerEncounterInterruptedByPeace();
				}
			}
	...
	public void ChangePartyLeader(Hero newLeader)
	{
		...
		{
			Debug.FailedAssert(string.Concat(newLeader?.Name, " is not a member of ", Name, "!\nParty leader did not change."), "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Party\\MobileParty.cs", "ChangePartyLeader", 1030);
		}
		else
		{
			if (IsLordParty)

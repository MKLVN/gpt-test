...
public class Campaign : GameType
{
	...
	internal void Tick()
	{
		...
		if (_dt > 0f || (MobileParty.MainParty.MapEvent == null && _curMapFrame == _lastNonZeroDtFrame + 1))
		{
			...
			if (Game.Current.GameStateManager.ActiveState is MapState mapState && mapState.AtMenu && !mapState.MenuContext.GameMenu.IsWaitActive)
			{
				_dt = 0f;
			}
		}
		...
		if (Game.Current.GameStateManager.ActiveState is MapState mapState2 && !mapState2.AtMenu)
		{
	...
	private void OnNewGameCreatedInternal()
	{
		...
		_newGameVersion = ApplicationVersion.FromParametersFile().ToString();

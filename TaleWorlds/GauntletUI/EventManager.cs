...
public class EventManager
{
	...
	public Widget DraggedWidget
	{
		get
		{
			return _draggedWidget;
		}
		private set
		{
			if (value != null && value.ConnectedToRoot)
			{
				_draggedWidget = value;
	...
	internal void OnFinalize()
	{
		...
		GauntletGamepadNavigationManager.Instance?.OnEventManagerFinalized(this);
	...
	internal void OnWidgetDisconnectedFromRoot(Widget widget)
	{
		...
		foreach (Widget allChildrenAndThi in widget.AllChildrenAndThis)
		{
			...
			GauntletGamepadNavigationManager.Instance?.OnWidgetDisconnectedFromRoot(this, allChildrenAndThi);
	...
	internal void MouseMove()
	{
		...
		foreach (Widget item in AllVisibleWidgetsAt(Root, MousePosition))
		{
			if (!MouseOveredViews.Contains(item))
			{
				...
				GauntletGamepadNavigationManager.Instance?.OnWidgetHoverBegin(this, item);
			}
			list.Add(item);
		}
	...
	{
		if (DragHoveredView != view && DragHoveredView != null)
		{
	...
	{
		return OnGetIsBlockedAtPosition(position);
	}
	...
	{
		return OnGetIsAvailableForGamepadNavigation();
	}
	...
	{
		GauntletGamepadNavigationManager.Instance?.OnEventManagerGainedNavigation(this);
	}

	public void GainNavigationAfterFrames(int frameCount, Func<bool> predicate = null)
	{
		GauntletGamepadNavigationManager.Instance?.SetEventManagerNavigationGainAfterFrames(this, frameCount, predicate);
	}

	public void GainNavigationAfterTime(float seconds, Func<bool> predicate = null)
	{
		GauntletGamepadNavigationManager.Instance?.SetEventManagerNavigationGainAfterTime(this, seconds, predicate);
	}

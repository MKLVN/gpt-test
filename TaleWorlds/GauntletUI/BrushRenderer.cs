	...
	public void Update(float globalAnimTime, float dt)
	{
		...
		if ((_brushRendererAnimationState == BrushRendererAnimationState.None || _brushRendererAnimationState == BrushRendererAnimationState.Ended) && (!string.IsNullOrEmpty(styleOfCurrentState.AnimationToPlayOnBegin) || _styleOfCurrentState.Version != _latestStyleVersion))
		{
			...
			_currentBrushState = brushState;
			{
				foreach (StyleLayer layer in styleOfCurrentState.Layers)
				{
					...
					value.FillFrom(layer);
					_currentBrushLayerState[layer.Name] = value;
					_startBrushLayerState[layer.Name] = value;
				}
				return;
			}
		}
		if (_brushRendererAnimationState == BrushRendererAnimationState.PlayingBasicTranisition)
		{
			...
			if (!(num >= Brush.TransitionDuration))
			{
				...
				{
					foreach (StyleLayer layer2 in styleOfCurrentState.Layers)
					{
						BrushLayerState start = _startBrushLayerState[layer2.Name];
						BrushLayerState value2 = default(BrushLayerState);
						value2.LerpFrom(start, layer2, num2);
						_currentBrushLayerState[layer2.Name] = value2;
					}
					return;
				}
			}
		...
		else
		{
			...
			if (animation != null && (animation.Loop || !(_brushTimer >= animation.Duration)))
			{
				...
				{
					foreach (StyleLayer layer3 in styleOfCurrentState.Layers)
					{
						BrushLayerState startState = _startBrushLayerState[layer3.Name];
						BrushLayerAnimation layerAnimation = animation.GetLayerAnimation(layer3.Name);
						BrushLayerState value3 = AnimateBrushLayerState(animation, layerAnimation, brushStateTimer, isFirstCycle, startState, layer3);
						_currentBrushLayerState[layer3.Name] = value3;
					}
					return;
				}
			}

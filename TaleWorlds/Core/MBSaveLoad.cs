...
public static class MBSaveLoad
{
	...
	public static bool IsUpdatingGameVersion => LastLoadedGameVersion < ApplicationVersion.FromParametersFile();
	...
	{
		ActiveSaveSlotName = null;
		LastLoadedGameVersion = ApplicationVersion.FromParametersFile();
		AutoSaveIndex = 0;
	}
	...
	private static void SaveGame(string saveName, MetaData metaData, Action<SaveResult> onSaveCompleted)
	{
		...
		catch (Exception ex)
		{
			...
			Debug.SilentAssert(ModuleHelper.GetModules().Any((ModuleInfo m) => !m.IsOfficial), ex.Message, getDump: false, "C:\\Develop\\MB3\\Source\\Bannerlord\\TaleWorlds.Core\\MBSaveLoad.cs", "SaveGame", 344);
		}
	}
	...
	private static MetaData GetSaveMetaData(CampaignSaveMetaDataArgs data)
	{
		...
		metaData.Add("ApplicationVersion", ApplicationVersion.FromParametersFile().ToString());

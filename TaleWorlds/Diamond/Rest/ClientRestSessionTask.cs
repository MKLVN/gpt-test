	...
	private void ExamineResult()
	{
		if (!Request.Successful)
		{
			...
			if (Request.Exception != null && Request.Exception is WebException)
			{
				...
				{
					TaleWorlds.Library.Debug.Print(string.Concat("Http Post Request(code:", code, ") with message(", RestRequestMessage, ") failed. Retrying. (", ex.Status, ")"));
					flag = true;
				}
				else
				{
					TaleWorlds.Library.Debug.Print(string.Concat("Http Post Request(code:", code, ") with message(", RestRequestMessage, ") failed. Exception status: ", ex.Status));
				}
			}
			else if (Request.Exception != null && Request.Exception is HttpPostRequestTimeoutException)
			{
